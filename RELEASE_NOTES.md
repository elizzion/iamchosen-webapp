...
1. **Flush Out Business Cycle Corporate Card**: Integrated the new "Flush Out Business Cycle" card inside the Financial Aggregates & Volume dashboard panel of the Super Admin Corporate Console.
2. **Dynamic Chosen Credits (CC) to PHP Conversion**: Displays aggregated flushed Business Cycle CC amounts as the primary value with dynamic PHP conversion ≈ ₱{amount} calculated below it using `system_config/cc_settings` rates (with a default fallback of ₱70 per 1 CC).
3. **Preferred and Fallback Collection Query Pathing**: Implemented relational queries prioritizing the `flushed_commissions` collection where `reason == "BUSINESS_CYCLE_COMPLETED"` and `status == "Flushed"`, falling back dynamically to `commissions` with identical filtering if the primary collection does not contain logs.
4. **Enhanced Security Restriction Controls**: Guarded card rendering to strictly enforce display limits only to authenticated users containing `role == "Super Admin"`.
5. **Flush Out Business Cycle Monitoring**: Implemented Flush Out Business Cycle monitoring with audit trails for commissions blocked by completed Business Cycles.
...
### Features Added & Changed
1. **Corporate Security Audit Trail Row Limits**: Added a limit dropdown to configure table row truncation at `50`, `100`, `250`, or `500` rows.
2. **Real-time Live Filter Search**: Implemented a comprehensive search interface to parse logs instantly by operator email, action type, or transaction descriptions.
3. **Dual Self-Maintaining Category Filters**: Added automated filters for administrative actions and operational operator emails that derive options dynamically from live database records.
4. **Dynamic Search and Count Feedback Indicators**: Designed detailed count metrics informing super administrators about matching log counts, active limits, and total events.
...
### Features Added & Changed
1. **Dynamic Firebase Configuration Load**: Replaced hardcoded client-side Firebase configuration properties in `src/firebase.ts` with direct, dynamic imports from `/firebase-applet-config.json` to automatically track changes in the client app registration or API keys.
2. **Fixed `auth/invalid-credential` Error**: Resolved the authentication failure code caused by stale or mismatched `appId` properties when connecting to Firebase Authentication endpoints.
3. **Compiler-Safe Type Resolution**: Added `"resolveJsonModule": true` to `tsconfig.json` to ensure clean, type-safe JSON resolution during build.
...
### Features Added & Changed
1. **Direct Downline Simulator CC Calculations**: Modified the calculation model to compute the Direct Downline Bonus using the CC package value instead of PHP sale values. Standardized with the business rule formula: `directDownlineBonusCC = packageValueCC * directReferralRate`.
2. **Real-time Rules Synchronization**: Subscribed to Firestore `system_config/business_rules.directReferralRate` with automatic fallback to 4%.
3. **All Affiliate/Distributor Packages Supported**: Rebuilt the Simulator UI to display and handle all 7 tiers (Bronze, Silver, Gold, Platinum, Diamond, City Distributor, Regional Distributor) with correct package values and bonus calculations.
4. **Complete PHP Sale Values Removal**: Cleared all previous hardcoded PHP sale values and references from the simulator layout to prevent developer or user confusion.
5. **Config Validation for Distributors**: Added automatic detection for City and Regional Distributor packages. Displays "Config required" if the package Value in CC is not found in the database.
6. **CC Precision Standard**: Ensured all simulator bonus output is strictly formatted up to a maximum of 2 decimal places.
7. **Unilevel Business Analytics Currency Conversion**: Replaced PHP fiat currency symbols and formulas with Chosen Credit (CC) units for Personal Sales and Monthly Earnings, formatting with comma separators. Fixed Bronze Dashboard analytics to ensure all sales, commissions, and cycle values are calculated and displayed in CC, not mislabeled PHP values. Programmed these metrics to dynamically compute from the live active downline members list (`downlineList`) for perfect, real-time consistency. Filtered the counting logic to strictly include only active affiliate packages ("Bronze", "Silver", "Gold", "Platinum", "Diamond", "City Distributor", "Regional Distributor") where role is "Affiliate" and status is "Active", preventing "None" packages, customers, or pending/inactive referrals from inflating affiliate figures.
8. **Unilevel Business Analytics Re-labeling**: Renamed the "Referral Count" / "Direct Members" label to "DIRECT AFFILIATE", formatting the display as "1 Member" / "X Members" with the subtitle "Direct Business Network".
9. **Zero-Safe Cycle Progress Handling**: Resolved undefined/NaN edge-cases in "Cycle Progress" calculations by implementing a robust fallback to 0% and constraining the bounds between 0% and 100%.
