...
## [1.7.4] - 2026-07-08 (Build 000030)

### Added
- **Flush Out Business Cycle Card**: Designed and integrated a high-priority "Flush Out Business Cycle" metric card inside the Super Admin Corporate Console under the Financial Aggregates & Volume bento grid. This card aggregates flushed Chosen Credits (CC) from completed affiliate Business Cycles and shows its PHP equivalent using `system_config/cc_settings` dynamic cash-in rate (with a fallback of 1 CC = ₱70). Restricts visibility strictly to "Super Admin" accounts.
- **Flush Out Business Cycle tracking**: Implemented Flush Out Business Cycle monitoring with audit trails for commissions blocked by completed Business Cycles.

## [1.7.3] - 2026-07-08 (Build 000029)

### Added
- **Advanced Corporate Security Audit Trail Controls**: Engineered a dynamic and responsive system audit control bar including:
  - Row limit configurations supporting `50`, `100`, `250`, and `500` rows.
  - Comprehensive live text searching through action names, actor details, and operator emails.
  - Self-maintaining action filter dropdown dynamically aggregated from active database logs.
  - Self-maintaining operational email filter dropdown dynamically aggregated from active database operators.
  - Dynamic result indicators showing matching vs. total database events with safe fallback alerts.

## [1.7.2] - 2026-07-08 (Build 000028)

### Fixed
- **Firebase Auth Configuration Dynamic Sync**: Replaced hardcoded Firebase config parameters in `src/firebase.ts` with direct, dynamic imports from `/firebase-applet-config.json`. This guarantees perfect synchronization of client `appId`, `apiKey`, and other configuration details, completely resolving the `auth/invalid-credential` errors caused by configuration mismatch on platform/database regenerations. Added `resolveJsonModule: true` to `tsconfig.json` to support compiler-safe JSON resolution.

## [1.7.1] - 2026-07-07 (Build 000027)

### Fixed
- **Direct Downline Simulator Bonus Calculations**: Re-engineered the calculation model to compute the Direct Downline Bonus using the CC package value instead of the PHP sale value. Uses the formula `directDownlineBonusCC = packageValueCC * directReferralRate` with a real-time Firestore subscriber to `system_config/business_rules.directReferralRate` and a safe fallback of 4%.
- **Dynamic Package Value Configuration**: Read packages dynamically from the Firestore `packages` collection where type is "Affiliate" or "Distributor", with complete, robust local fallback definitions for Bronze, Silver, Gold, Platinum, and Diamond tiers.
- **Simulator UI Package Rows Expansion**: Expanded and redesigned the Simulator interface to show all 7 affiliate and distributor packages: Bronze, Silver, Gold, Platinum, Diamond, City Distributor, and Regional Distributor, displaying correct package values and calculated bonuses.
- **PHP Sale Value Removal**: Completely removed all incorrect PHP-based sale values and references from the Direct Downline Simulator UI, in compliance with security guidelines and system calculations.
- **Distributor Config Validation**: Added safe "Config required" state validation for City Distributor and Regional Distributor tiers when their configuration is missing from Firestore database.
- **CC Precision Formatting**: Standardized all computed Chosen Credit (CC) values inside the simulator to display with a maximum of 2 decimal places.
- **Unilevel Business Analytics Card Currency & Calculations**: Updated the Personal Sales and Monthly Earnings displays to show values strictly in Chosen Credits (CC) instead of PHP currency, formatting them with thousand separators (e.g. 150 CC, 62 CC). Fixed Bronze Dashboard analytics to ensure all sales, commissions, and cycle values are calculated and displayed in CC, not mislabeled PHP values. Made the metrics fully dynamic, deriving directly from the filtered live downline members list (`downlineList`) to ensure perfect alignment with the actual downline network. Programmed the counting logic to strictly include only active affiliate packages ("Bronze", "Silver", "Gold", "Platinum", "Diamond", "City Distributor", "Regional Distributor") where role is "Affiliate" and status is "Active", preventing any "None" package, customer, or inactive/pending users from inflating the affiliate metrics.
- **Unilevel Business Analytics Card Labels**: Renamed the "Referral Count" / "Direct Members" label to "DIRECT AFFILIATE" with formatted display (e.g. "1 Member", "X Members") and updated the subtitle to "Direct Business Network".
- **Unilevel Business Analytics Card Cycle Progress Fallback**: Added robust fallback handling for "Cycle Progress" to compute from the real formula `(currentQualifiedEarningsCC / earningsCapCC) * 100`, strictly resolving to 0% if missing, null, undefined, NaN, or invalid, preventing any display of "undefined%" or "NaN%".
