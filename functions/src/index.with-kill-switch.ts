...
async function getCCRate(): Promise<number> {
  ...
  const rate = Number(
    data.purchaseRatePHP ??
    data.cashInRatePHP ??
    data.displayReferenceRatePHP
  );
...
export const purchasePackageWithWallet = onCall({ region: "asia-southeast1" }, async (request: any) => {
  ...
  try {
    ...
    const transactionResult = await db.runTransaction(async (transaction: Transaction) => {
...
function displayMemberId(value: unknown): string {
  ...
  return normalized.startsWith("IAM-")
    ? normalized
    : `IAM-${normalized}`;
}
...
export const executeP2PTransferV2 = onCall<ExecuteP2PTransferRequest>(
  ...
  async (
    ...
    const normalizedRecipientMemberId =
      normalizeMemberId(recipientMemberId);

    if (
      typeof amountCC !== "number" ||
      !Number.isFinite(amountCC)
    ) {
      throw new HttpsError(
        "invalid-argument",
    ...
    const normalizedAmountCC =
      Math.round(amountCC * 10000) / 10000;
    ...
    const canonicalIdempotencyPattern =
      /^P2P-TX-\d{10,17}-[A-Z0-9]{6,16}$/;
    ...
    try {
      ...
      const recipientUid =
        recipientQuerySnapshot.docs[0].id;
      ...
      const transferResult = await db.runTransaction(
        async (transaction: Transaction): Promise<ExecuteP2PTransferResult> => {
          const transferRef = db
            .collection("p2p_transfers")
            .doc(idempotencyKey);

          const senderUserRef = db
            .collection("users")
            .doc(senderUid);
          ...
          const senderWalletRef = db
            .collection("wallets")
            .doc(senderUid);

          const recipientWalletRef = db
            .collection("wallets")
            .doc(recipientUid);
          ...
          const transferSnapshot =
            await transaction.get(transferRef);
          ...
          if (transferSnapshot.exists) {
            ...
            const legacyRequestMatches =
              ...
              normalizeMemberId(
                String(existing.recipientMemberId || "")
              ) === normalizedRecipientMemberId &&
            ...
            if (
              ...
              return {
                ...
                feeTransactionId:
                  String(existing.feeTransactionId || ""),
                ...
                totalDebitCC: Number(
                  existing.totalDebitCC
                ),
                recipientName: String(
                  existing.recipientName || ""
                ),
                recipientId: String(
                  existing.recipientMemberId ||
                  normalizedRecipientMemberId
                ),
          ...
          if (!senderWalletSnapshot.exists) {
            throw new Error(
              "SENDER_WALLET_NOT_INITIALIZED"
            );
          }

          if (!recipientWalletSnapshot.exists) {
            throw new Error(
              "RECIPIENT_WALLET_NOT_INITIALIZED"
            );
          }
          ...
          const senderUser =
            senderUserSnapshot.data() || {};

          const recipientUser =
            recipientUserSnapshot.data() || {};

          const senderWallet =
            senderWalletSnapshot.data() || {};

          const recipientWallet =
            recipientWalletSnapshot.data() || {};

          const ccSettings =
            ccSettingsSnapshot.data() || {};

          const storedRecipientMemberId =
            normalizeMemberId(
          ...
          if (
            !isActiveAccountStatus(recipientUser.status)
          ) {
          ...
          // Server-authoritative P2P kill switch. The frontend feature flag is
          // not a security boundary; production transfers remain disabled unless
          // this exact Boolean field is explicitly enabled in cc_settings.
          ...
          const feeCC = Number(
            ccSettings.p2pTransferFeeCC
          );

          if (
            !Number.isFinite(feeCC) ||
            feeCC <= 0
          ) {
          ...
          if (
            ...
            throw new Error(
              "INVALID_FEE_CONFIG_VERSION"
            );
          }

          const senderBalance = Number(
            senderWallet.chosenWalletBalance
          );

          const recipientBalance = Number(
            recipientWallet.chosenWalletBalance
          );
          ...
          if (
            ...
            throw new Error(
              "INVALID_RECIPIENT_WALLET_BALANCE"
            );
          }
          ...
          const senderTransferLedgerId =
            deterministicP2PId(
              idempotencyKey,
              "TX-P2P-SENDER"
            );

          const senderFeeLedgerId =
            deterministicP2PId(
              idempotencyKey,
              "TX-P2P-FEE"
            );

          const recipientCreditLedgerId =
            deterministicP2PId(
              idempotencyKey,
              "TX-P2P-RECIPIENT"
            );

          const treasuryCreditLedgerId =
            deterministicP2PId(
              idempotencyKey,
              "TX-P2P-TREASURY"
            );
          ...
          transaction.update(senderWalletRef, {
            chosenWalletBalance:
              newSenderBalance,
            walletVersion:
              Number(senderWallet.walletVersion || 0) + 1,
            updatedAt: timestamp
          });
          ...
          transaction.update(recipientWalletRef, {
            chosenWalletBalance:
              newRecipientBalance,
            walletVersion:
              Number(recipientWallet.walletVersion || 0) + 1,
            updatedAt: timestamp
          });
          ...
          if (treasurySnapshot.exists) {
            transaction.update(treasuryRef, {
              ...
              treasuryVersion:
                Number(
                  treasuryData.treasuryVersion || 0
                ) + 1,
              updatedAt: timestamp
            });
          } else {
            transaction.set(treasuryRef, {
              id:
                TECHNOLOGY_OPERATIONS_TREASURY_ID,
              displayName:
                "Technology Operations Treasury",
              accountType: "SYSTEM_TREASURY",
              classification:
                "CORPORATE_TECHNOLOGY_REVENUE",
              status: "ACTIVE",
              balanceCC: newTreasuryBalance,
              treasuryVersion: 1,
          ...
          transaction.set(
              ...
              .doc(senderTransferLedgerId),
            {
              ...
              description:
                `Transfer to ${displayMemberId(
                  storedRecipientMemberId
                )}`,
              idempotencyKey:
                `${idempotencyKey}:sender-transfer-debit`,
              createdAt: timestamp,
              completedAt: timestamp
            }
          );
          ...
          transaction.set(
              ...
              .doc(senderFeeLedgerId),
            {
              ...
              referenceNumber: idempotencyKey,
              description:
                `Platform Transfer Fee for transfer ${idempotencyKey}`,
              idempotencyKey:
                `${idempotencyKey}:sender-fee-debit`,
              createdAt: timestamp,
              completedAt: timestamp
            }
          );
          ...
          transaction.set(
              ...
              .doc(recipientCreditLedgerId),
            {
              ...
              referenceNumber: idempotencyKey,
              description:
                `Transfer from ${displayMemberId(
                  senderUser.memberId
                )}`,
              idempotencyKey:
                `${idempotencyKey}:recipient-transfer-credit`,
              createdAt: timestamp,
              completedAt: timestamp
            }
          );
          ...
          transaction.set(
              ...
              .doc(treasuryCreditLedgerId),
            {
              ...
              description:
                `Platform Transfer Fee for transfer ${idempotencyKey}`,
              idempotencyKey:
                `${idempotencyKey}:treasury-fee-credit`,
              createdAt: timestamp,
              completedAt: timestamp
            }
          );
          ...
          transaction.set(
            db.collection("transfer_fee_transactions")
              .doc(feeTransactionId),
            {
              feeTransactionId,
              sourceTransferId: idempotencyKey,
              sourceTransferType: "P2P_TRANSFER",
              ...
              feeType:
                "PLATFORM_TRANSFER_FEE",
              feeAmountType: "FIXED",
              feeAmountCC: feeCC,
              transferAmountCC:
                normalizedAmountCC,
              totalSenderDeductionCC:
                totalDebitCC,
              destinationTreasuryId:
                TECHNOLOGY_OPERATIONS_TREASURY_ID,
              ...
              internalClassification:
                "CORPORATE_TECHNOLOGY_REVENUE",
              senderFeeLedgerTransactionId:
                senderFeeLedgerId,
              treasuryLedgerTransactionId:
                treasuryCreditLedgerId,
              configurationVersion:
                configVersion,
              idempotencyKey:
                `platform-transfer-fee:${idempotencyKey}`,
              status: "Completed",
      ...
      console.info("P2P transfer completed", {
        ...
        duplicate:
          transferResult.isDuplicate === true,
      ...
      if (
        errorMessage ===
          "MISSING_CC_SETTINGS" ||
        errorMessage ===
          "INACTIVE_CC_SETTINGS" ||
        errorMessage ===
          "INACTIVE_TRANSFER_FEE" ||
        errorMessage ===
          "INVALID_FEE_VALUE" ||
        errorMessage ===
          "FEE_VALUE_CONFLICT" ||
