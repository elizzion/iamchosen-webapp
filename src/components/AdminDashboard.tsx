...
function normalizeCashInRequest(
  ...
  return {
    requestId: readString(data.requestId, data.id, documentId) || documentId,
    uid: readString(data.uid),
    fullName: readString(data.fullName) || 'Unknown Member',
    amountCC: readNumber(data.amountCC, data.computedCC),
    amountPHP: readNumber(data.amountPHP, data.amountPhp),
    ...
    paymentMethod:
      readString(data.paymentMethod, data.paymentChannel) || 'Unavailable',
    referenceNumber: readString(data.referenceNumber) || 'Unavailable',
    proofOfPaymentUrl: readString(data.proofOfPaymentUrl),
...
export default function AdminDashboard({
  ...
  const [cashinRequests, setCashinRequests] = useState<CashInRequestRow[]>([])
  ...
  useEffect(() => {
    if (error) {
      window.showError?.(error, 'Admin Action Error')
    }
  }, [error])

  useEffect(() => {
    if (success) {
      window.showSuccess?.(success, 'Admin Action Succeeded')
    }
  }, [success])
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      const cashinList = cashinSnapshot.docs.map((documentSnapshot) =>
        normalizeCashInRequest(documentSnapshot.id, documentSnapshot.data()),
      )
      cashinList.sort((first, second) => {
      ...
      const custReqList = custReqSnapshot.docs.map(
      ...
      const affReqList = affReqSnapshot.docs.map(
        (doc) => ({ id: doc.id, ...doc.data() }) as any,
      )
      affReqList.sort(
        (a, b) =>
      ...
      const calculatedStats = userList.reduce(
        (acc, user: any) => {
          acc.totalUsers += 1
          ...
          else if (user.role === 'City Distributor')
            acc.totalCityDistributors += 1
          else if (user.role === 'Regional Distributor')
            acc.totalRegionalDistributors += 1

          if (
            user.status === 'Active' &&
            user.packageLevel &&
            user.packageLevel !== 'None'
          ) {
            acc.activePackagesCount += 1
            const level = user.packageLevel as
              | 'Bronze'
              | 'Silver'
              | 'Gold'
              | 'Platinum'
  ...
  const handleApproveCashin = async (requestId: string): Promise<void> => {
    ...
    try {
      // All Cash-In approvals are authoritative server-side operations.
      // The callable function validates the request amount, member UID,
      // idempotency, wallet balance update, and immutable ledger entry.
      await approveCashInAndActivatePackage({ requestId })

      setSuccess(
        `Cash-In request ${requestId} was approved and credited successfully.`,
      )
      await fetchAdminData()
    } catch (err: unknown) {
  ...
  const handleApproveCustomerPackageRequest = async (
    requestId: string,
    uid: string,
    packageName: string,
    ccAmount: number,
    ...
    try {
      ...
      setSuccess(
        `Successfully approved Customer Package Request and updated package level to ${packageName}!`,
      )
      await fetchAdminData()
    } catch (err: any) {
      console.error(err)
  ...
  const handleRejectCustomerPackageRequest = async (
    requestId: string,
    reason: string,
  ) => {
    if (!reason.trim()) {
  ...
  const handleApproveAffiliateActivationRequest = async (
    requestId: string,
    uid: string,
    packageName: string,
    ccVal: number,
    ...
    try {
      ...
      setSuccess(
        `Successfully approved Affiliate Activation! User role updated to Affiliate and business cycle initialized.`,
      )
      await fetchAdminData()
    } catch (err: any) {
      console.error(err)
  ...
  const handleRejectAffiliateActivationRequest = async (
    requestId: string,
    reason: string,
  ) => {
    if (!reason.trim()) {
  ...
  const pendingRegistrations = users.filter(
    (u: any) =>
      u.status === 'Inactive' && u.paymentStatus === 'Pending Approval',
  )
  const pendingCashinRequests = cashinRequests.filter(
    (c: any) => c.status === 'Pending' || c.status === 'Submitted',
  )
      ...
      <main className='max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4 sm:pt-6 lg:pt-6 pb-10'>
        ...
        <div className='mb-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4'>
          <div>
            ...
            <p className='text-zinc-400 font-light'>
              Overview, member management, cash-ins, registrations, and catalog
              placeholders.
            </p>
          </div>
        ...
        <div className='flex flex-wrap border-b border-zinc-900 gap-1 mb-8'>
          <button
            ...
            className={`px-5 py-3 text-xs uppercase tracking-wider font-extrabold transition-all border-b-2 cursor-pointer ${
              activeTab === 'metrics'
                ? 'border-gold text-gold bg-zinc-900/20'
          ...
          <div className='space-y-8 animate-fadeIn'>
            ...
            <div className='grid grid-cols-2 md:grid-cols-5 gap-4'>
              ...
              <div className='bg-zinc-950 border border-zinc-900 p-5 rounded-xl shadow-md'>
                ...
                <span className='text-2xl font-black text-white block mt-1'>
                  {stats.totalRegionalDistributors}
                </span>
              </div>
            </div>
            ...
            <div>
              <h2 className='text-xs uppercase tracking-widest text-zinc-500 font-black mb-4'>
                Financial Overview
              </h2>
              <div className='grid grid-cols-1 sm:grid-cols-4 gap-4'>
                ...
                <div className='bg-zinc-950 border border-zinc-900 p-5 rounded-xl'>
                  ...
                  <span className='text-xl font-black text-white block mt-1'>
                    {stats.ccConsumedCC.toLocaleString()} CC
                  </span>
                </div>
                <div className='bg-zinc-950 border border-zinc-900 p-5 rounded-xl'>
                  ...
                  <span className='text-xl font-black text-white block mt-1'>
                    {stats.referralBonusesCC.toLocaleString()} CC
                  </span>
                </div>
                <div className='bg-zinc-950 border border-zinc-900 p-5 rounded-xl'>
            ...
            <div className='bg-zinc-950 border border-zinc-900 p-6 rounded-xl'>
              <h3 className='font-extrabold text-sm text-zinc-300 uppercase tracking-wide mb-4'>
                Packages Breakdown
              </h3>
              <div className='grid grid-cols-5 gap-4'>
                <div className='text-center p-3 bg-zinc-900 rounded-lg'>
                  ...
                  <span className='block text-lg font-black text-white mt-1'>
                    {stats.activePackagesBreakdown.Bronze}
                  </span>
                </div>
                <div className='text-center p-3 bg-zinc-900 rounded-lg'>
                  <span className='block text-[10px] text-zinc-500 font-bold uppercase'>
                  ...
                  <span className='block text-lg font-black text-white mt-1'>
                    {stats.activePackagesBreakdown.Silver}
                  </span>
                </div>
                <div className='text-center p-3 bg-zinc-900 rounded-lg'>
                  <span className='block text-[10px] text-zinc-500 font-bold uppercase'>
                  ...
                  <span className='block text-lg font-black text-white mt-1'>
                    {stats.activePackagesBreakdown.Gold}
                  </span>
                </div>
                <div className='text-center p-3 bg-zinc-900 rounded-lg'>
                  <span className='block text-[10px] text-zinc-500 font-bold uppercase'>
                  ...
                  <span className='block text-lg font-black text-white mt-1'>
                    {stats.activePackagesBreakdown.Platinum}
                  </span>
                </div>
                <div className='text-center p-3 bg-zinc-900 rounded-lg'>
                  <span className='block text-[10px] text-zinc-500 font-bold uppercase'>
                  ...
                  <span className='block text-lg font-black text-white mt-1'>
                    {stats.activePackagesBreakdown.Diamond}
                  </span>
                </div>
              </div>
            </div>
        ...
        {activeTab === 'members' && (
          <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-6 animate-fadeIn'>
            <h3 className='font-extrabold text-lg text-white uppercase tracking-tight flex items-center gap-2 mb-6'>
              <Users className='w-5 h-5 text-gold' /> Registered System Users (
              {users.length})
            </h3>
                  ...
                  <tbody className='text-zinc-300 font-light'>
                    {cashinRequests.map((req, idx) => (
                      ...
                      >
                        ...
                        <td className='py-3.5 px-4'>{req.paymentMethod}</td>
                        <td className='py-3.5 px-4 font-mono text-zinc-400'>
                          {req.referenceNumber}
                        </td>
                        <td className='py-3.5 px-4'>
                        ...
                        <td className='py-3.5 px-4 text-right'>
                          {(req.status === 'Pending' ||
                            ...
                            <div className='flex justify-end gap-1.5'>
                              ...
                              <button
                                onClick={() =>
                                  handleDeclineCashin(req.requestId, req.uid)
                                }
                                disabled={actionLoading !== null}
          ...
          <div className='space-y-8 animate-fadeIn'>
            ...
            <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-6'>
              <h3 className='font-extrabold text-lg text-white uppercase tracking-tight flex items-center gap-2 mb-2'>
                <Crown className='w-5 h-5 text-gold' /> Pending Affiliate
                Activations
              </h3>
              <p className='text-zinc-500 text-xs mb-6'>
                Requests from Customers selecting the Affiliate Business path.
              </p>
                      ...
                      {affiliateRequests
                        ...
                        .map((r, idx) => (
                          ...
                          >
                            ...
                            <td className='py-3.5 px-4 text-gold font-extrabold uppercase'>
                              {r.selectedPackage}
                            </td>
                            <td className='py-3.5 px-4 font-mono text-zinc-200'>
                              {r.packageValueCC} CC{' '}
                              <span className='text-[10px] text-zinc-500'>
                                (₱{(r.amountPHP || 0).toLocaleString()})
                              </span>
                            </td>
                            ...
                            <td className='py-3.5 px-4 text-right space-x-2'>
                              <button
                                onClick={() =>
                                  handleApproveAffiliateActivationRequest(
                                    r.id,
                                    r.uid,
                                    r.selectedPackage,
                                    r.packageValueCC,
            ...
            <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-6'>
              <h3 className='font-extrabold text-lg text-white uppercase tracking-tight flex items-center gap-2 mb-2'>
                <ShoppingBag className='w-5 h-5 text-emerald-400' /> Pending
                Customer Packages
              </h3>
              <p className='text-zinc-500 text-xs mb-6'>
                Product package purchases from customers on the Smart Customer
                path.
              </p>
                        ...
                        .map((r, idx) => (
                          ...
                          >
                            ...
                            <td className='py-3.5 px-4 text-right space-x-2'>
                              <button
                                onClick={() =>
                                  handleApproveCustomerPackageRequest(
                                    r.id,
                                    r.uid,
                                    r.selectedPackage,
                                    r.amountCC,
            ...
            <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-6'>
              <h3 className='font-extrabold text-lg text-white uppercase tracking-tight flex items-center gap-2 mb-2'>
                <Award className='w-5 h-5 text-cyan-400' /> Pending Public
                Registrations
              </h3>
              <p className='text-zinc-500 text-xs mb-6'>
                Traditional direct onboarding pending activations.
              </p>
                      ...
                      {pendingRegistrations.map((u, idx) => (
                        ...
                        >
                          ...
                          <td className='py-3.5 px-4'>
                            <button
                              onClick={() =>
                                handleApproveAffiliate(u.uid, u.fullName)
                              }
                              disabled={actionLoading !== null}
              ...
              <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm'>
                <div className='bg-[#0B0D12] border border-zinc-800/95 rounded-2xl max-w-sm w-full p-5 space-y-4 shadow-2xl'>
                  ...
                  <p className='text-zinc-400 text-xs'>
                    Please provide an official reason for declining request{' '}
                    ...
                    <button
                      onClick={() => {
                        if (rejectingType === 'customer') {
                          handleRejectCustomerPackageRequest(
                            rejectingRequestId,
                            rejectionReasonInput,
                          )
                        } else {
                          handleRejectAffiliateActivationRequest(
                            rejectingRequestId,
                            rejectionReasonInput,
                          )
                        }
        ...
        {activeTab === 'products' && (
          <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-8 text-center animate-fadeIn'>
            ...
            <h3 className='font-extrabold text-white uppercase text-md mb-2'>
              Corporate Product Catalog Manager
            </h3>
            <p className='text-zinc-500 text-xs max-w-md mx-auto mb-6 leading-relaxed'>
              Standard Admins can track inventories, adjust SKU codes, and
              restock units. Complete CRUD services are configured.
            </p>
        ...
        {activeTab === 'orders' && (
          <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-8 text-center animate-fadeIn'>
            ...
            <p className='text-zinc-500 text-xs max-w-md mx-auto mb-6 leading-relaxed'>
              Track customer purchases, package shipments, and order logs.
              Standard Admins have access to update status to "Shipped" or
              "Delivered".
            </p>
        ...
        {activeTab === 'reports' && (
          <div className='bg-zinc-950 border border-zinc-900 rounded-xl p-8 text-center animate-fadeIn'>
            ...
            <p className='text-zinc-500 text-xs max-w-md mx-auto leading-relaxed'>
              Generate PDF and CSV summaries for registration logs, wallet
              ledgers, tax withholdings, and total unilevel cycles.
            </p>
          </div>
        )}
      </main>
