...
import { db, approvePendingAffiliate, createAuditLog, approveCashInAndActivatePackage } from '../firebase';
...
export default function AdminDashboard({ onNavigate, currentUserProfile }: AdminDashboardProps) {
  ...
  const handleApproveCashin = async (requestId: string, uid: string, amountCC: number) => {
    ...
    setSuccess(null);
    try {
      const reqObj = cashinRequests.find((c: any) => c.requestId === requestId);
      if (reqObj && reqObj.packageId) {
        // This is a package purchase cash-in request. Call the backend callable function!
        await approveCashInAndActivatePackage({ requestId });
        setSuccess(`Successfully approved package purchase and activated user affiliate account!`);
        await fetchAdminData();
        return;
      }
