...
export default function PackageSelectionPage({
  ...
  const [typeParam, setTypeParam] = useState<string | null>(() => {
    if (typeof window !== 'undefined') {
      ...
      try {
        ...
        if (stored === 'smart-customer' || stored === 'affiliate-business') {
          return stored
        }
      } catch (error) {
        console.error('Unable to read the selected package path.', error)
      }
    }
    ...
    if (
      ...
      userProfile?.role === 'City Distributor' ||
      userProfile?.role === 'Regional Distributor'
    ) {
      return 'affiliate-business'
    }

    if (
      userProfile?.accountType === 'SmartCustomer' ||
      String(userProfile?.accountType || '').trim() === 'Smart Customer'
    ) {
  ...
  const [selectedPackageToConfirm, setSelectedPackageToConfirm] = useState<
    SmartCustomerPackage | AffiliatePackage | null
  >(null)
  ...
  const [successRedirectSeconds, setSuccessRedirectSeconds] = useState(4)
    ...
    switch (name) {
      case 'Bronze':
        return {
          ...
          btnGradient:
            'from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-black shadow-[0_4px_12px_rgba(245,158,11,0.15)] focus:ring-amber-500/30',
          accentBorder:
            'hover:border-amber-600/40 hover:shadow-[0_0_25px_rgba(245,158,11,0.08)]',
      ...
      case 'Silver':
        return {
          ...
          btnGradient:
            'from-zinc-500 to-slate-600 hover:from-zinc-400 hover:to-slate-500 text-white shadow-[0_4px_12px_rgba(156,163,175,0.15)] focus:ring-zinc-500/30',
          accentBorder:
            'hover:border-zinc-500/40 hover:shadow-[0_0_25px_rgba(156,163,175,0.08)]',
      ...
      case 'Gold':
        return {
          ...
          btnGradient:
            'from-amber-500 to-[#D4AF37] hover:from-amber-400 hover:to-[#E5C158] text-black shadow-[0_4px_15px_rgba(212,175,55,0.25)] focus:ring-amber-500/30',
          accentBorder:
            'border-[#D4AF37]/30 hover:border-[#D4AF37]/60 hover:shadow-[0_0_35px_rgba(212,175,55,0.15)]',
      ...
      case 'Platinum':
        return {
          ...
          btnGradient:
            'from-sky-500 to-indigo-600 hover:from-sky-400 hover:to-indigo-500 text-white shadow-[0_4px_12px_rgba(56,189,248,0.15)] focus:ring-sky-500/30',
          accentBorder:
            'hover:border-sky-500/40 hover:shadow-[0_0_25px_rgba(56,189,248,0.08)]',
          watermarkIcon: Award,
          highlighted: false,
        }
      case 'Diamond':
        return {
          ...
          btnGradient:
            'from-cyan-500 to-teal-600 hover:from-cyan-400 hover:to-teal-500 text-black shadow-[0_4px_12px_rgba(6,182,212,0.25)] focus:ring-cyan-500/30',
          accentBorder:
      ...
      case 'City Distributor':
        return {
          ...
          btnGradient:
            'from-violet-500 to-[#8B5CF6] hover:from-violet-400 hover:to-[#A78BFA] text-white shadow-[0_4px_12px_rgba(139,92,246,0.25)] focus:ring-violet-500/30',
          accentBorder:
            'hover:border-violet-500/40 hover:shadow-[0_0_25px_rgba(139,92,246,0.08)]',
          watermarkIcon: Network,
          highlighted: false,
        }
      case 'Regional Distributor':
        return {
          ...
          btnGradient:
            'from-rose-500 to-[#F43F5E] hover:from-rose-400 hover:to-[#FB7185] text-white shadow-[0_4px_12px_rgba(244,63,94,0.25)] focus:ring-rose-500/30',
          accentBorder:
            'hover:border-rose-500/40 hover:shadow-[0_0_25px_rgba(244,63,94,0.08)]',
          watermarkIcon: Crown,
          highlighted: false,
        }
      default:
  ...
  useEffect(() => {
    ...
    const storedType = sessionStorage.getItem('selectedPath')

    const resolvedType =
      queryType === 'smart-customer' || queryType === 'affiliate-business'
        ? queryType
        : storedType === 'smart-customer' || storedType === 'affiliate-business'
          ? storedType
          : userProfile?.accountType === 'Affiliate' ||
              userProfile?.role === 'Affiliate' ||
              userProfile?.role === 'City Distributor' ||
            ...
            ? 'affiliate-business'
            : userProfile?.accountType === 'SmartCustomer' ||
                String(userProfile?.accountType || '').trim() ===
                  'Smart Customer'
              ? 'smart-customer'
              : null

    setTypeParam(resolvedType)
    ...
    const loadData = async () => {
      ...
      try {
        ...
        console.error(
          'Error fetching business cycle in PackageSelectionPage:',
          err,
        )
      } finally {
        setFetchingCycle(false)
  ...
  const returnToDashboardAfterSuccess = useCallback(() => {
    ...
    // Reload after navigation so the parent profile, package, wallet, Business
    // Cycle, rank, and network summary are reloaded from Firestore.
    window.setTimeout(() => {
      window.location.reload()
    }, 150)
  }, [onNavigate, successData])

  useEffect(() => {
    if (!successData) return

    setSuccessRedirectSeconds(4)
    setSuccessRedirecting(false)

    const countdownTimer = window.setInterval(() => {
      setSuccessRedirectSeconds((current) => Math.max(1, current - 1))
    }, 1000)

    const redirectTimer = window.setTimeout(() => {
      returnToDashboardAfterSuccess()
    }, 4000)
  ...
  }, [successData, returnToDashboardAfterSuccess])
    ...
    return (
      <div className='bg-black min-h-screen text-white flex flex-col items-center justify-center p-6'>
        ...
        <button
          ...
          className='px-5 py-2.5 bg-gold text-black font-bold rounded-xl'
        >
          Go to Login
        </button>
      </div>
  ...
  const affiliatePackages: AffiliatePackage[] = [
    ...
    },
    {
      ...
      cap: 62500,
      description:
        'Exclusive city distribution hub with highest commission privileges.',
      benefits: [
    ...
    },
    {
      ...
      cap: 250000,
      description:
        'The highest operational tier with regional distribution monopoly.',
      benefits: [
  ...
  const smartCustomerPackages: SmartCustomerPackage[] = [
    {
      ...
      php: 1400,
      description:
        'Ideal for individuals starting their premium wellness journey.',
      benefits: [
    ...
    },
    {
      ...
      php: 4200,
      description:
        'Best for families seeking consistent nutrition and lifestyle support.',
      benefits: [
    ...
    },
    {
      ...
      php: 10500,
      description:
        'Our most complete bundle of specialized concentrates and extracts.',
      benefits: [
  ...
  const resolveActivationAction = (
    selectedPackageName: string,
  ): PackageActivationAction => {
    const currentPackage = String(userProfile.packageLevel || 'None').trim()

    if (!currentPackage || currentPackage === 'None') {
      return 'INITIAL_ACTIVATION'
    }

    const selectedIsCurrentPackage =
      selectedPackageName.trim().toLowerCase() === currentPackage.toLowerCase()

    if (businessCycle?.status === 'Completed' && selectedIsCurrentPackage) {
      return 'BUSINESS_CYCLE_REACTIVATION'
    }

    return 'PACKAGE_UPGRADE'
  }

  const handleInitiatePackagePreview = async (
    pkg: SmartCustomerPackage | AffiliatePackage,
  ) => {
    setLoading(true)
    setError(null)
    setPreviewData(null)
    ...
    try {
      ...
      if (data && data.success) {
        ...
        setError(
          'Failed to fetch authoritative purchase preview from the backend.',
        )
      }
    } catch (err: any) {
  ...
  const handleExecutePurchase = async () => {
    if (!selectedPackageToConfirm || !previewData || loading) return

    setLoading(true)
    setError(null)

    const authenticatedUid = auth.currentUser?.uid || ''
    if (!authenticatedUid || authenticatedUid !== userProfile.uid) {
      setError(
        'Your authenticated session does not match this member profile. Please sign in again.',
      )
      setLoading(false)
      return
    }
    ...
    const isAffiliate = typeParam === 'affiliate-business'
    const accountPath = isAffiliate ? 'Affiliate' : 'Smart Customer'
    const activationAction = resolveActivationAction(
      selectedPackageToConfirm.name,
    )

    try {
      const data = await PackageService.activatePackage({
      ...
      if (onProfileUpdate) {
        onProfileUpdate({
          ...
          accountType: (isAffiliate
            ? 'Affiliate'
            : 'SmartCustomer') as UserProfile['accountType'],
          packageLevel: data.packageLevel as UserProfile['packageLevel'],
          status: 'Active',
      ...
      sessionStorage.removeItem('packageAccountPath')
      sessionStorage.removeItem('packageActivationAction')
      ...
      console.error('Error in activatePackageWithWallet callable:', err)
      setError(
        err instanceof Error && err.message
          ? err.message
          : 'The secured package engine could not complete this request.',
      )
    } finally {
      setLoading(false)
    }
  ...
  const handleBack = () => {
    ...
    onNavigate(
      isAffiliateAccount ? 'affiliate-dashboard' : 'customer-dashboard',
    )
  }
      ...
      <div className='bg-black text-white min-h-screen flex flex-col justify-between selection:bg-gold selection:text-black'>
        ...
        <div className='flex-1 flex flex-col items-center justify-center p-6 text-center'>
          ...
          <p className='text-zinc-400 text-sm max-w-md leading-relaxed mb-8'>
            Please choose your path first before proceeding to package
            selection.
          </p>
        ...
        <div className='p-6 text-center text-[10px] text-zinc-600 font-mono tracking-wider'>
          IAM CHOSEN INTERNATIONAL • PORTAL V1.6.0
        </div>
      </div>
    )
  }
  ...
  const getPackageOrder = (pkgName: string): number => {
    if (dbPackages && dbPackages.length > 0) {
      const found = dbPackages.find(
        (p) =>
          (p.name || p.id || '').toLowerCase().trim() ===
          pkgName.toLowerCase().trim(),
      )
  ...
  const affiliatePackageNames = new Set([
    'bronze',
    'silver',
    'gold',
    'platinum',
  ...
  const smartCustomerOrder: Record<string, number> = {
  ...
  dbPackages.forEach((dbPkg) => {
    ...
    const existingIdx = mergedAffiliatePackages.findIndex(
      (p) => p.name.toLowerCase().trim() === name.toLowerCase().trim(),
    )

    // Check if it's an Affiliate/Distributor type pkg
    const isAffiliateType =
      dbPkg.type === 'Affiliate' ||
      dbPkg.type === 'Distributor' ||
      [
        'bronze',
        'silver',
        'gold',
    ...
    if (isAffiliateType) {
      const mappedPkg: AffiliatePackage = {
        name: name,
        cc:
          typeof dbPkg.packageValueCC === 'number'
            ? dbPkg.packageValueCC
            : typeof dbPkg.cc === 'number'
              ? dbPkg.cc
              : Number(dbPkg.packageValueCC || dbPkg.cc || 0),
        php:
          typeof dbPkg.pricePHP === 'number'
            ? dbPkg.pricePHP
            : typeof dbPkg.php === 'number'
              ? dbPkg.php
              : Number(dbPkg.pricePHP || dbPkg.php || 0),
        cap:
          typeof dbPkg.earningsCapCC === 'number'
            ? dbPkg.earningsCapCC
            : typeof dbPkg.cap === 'number'
              ? dbPkg.cap
              : Number(dbPkg.earningsCapCC || dbPkg.cap || 0),
  ...
  const visibleAffiliateTiers = sortedAffiliatePackages.filter((pkg) => {
    const userLevel = String(userProfile?.packageLevel || 'None').trim()
    const normalizedUserLevel = userLevel.toLowerCase()
    const currentIsAffiliatePackage =
      affiliatePackageNames.has(normalizedUserLevel)
    const pkgOrder = getPackageOrder(pkg.name)

    // A Basic Customer or Smart Customer moving to Affiliate Business starts
    // from the standard Affiliate package range. Distributor tiers remain
    // hidden during this cross-path activation.
    if (userLevel === 'None' || !currentIsAffiliatePackage) {
      return pkgOrder <= 5
    }
    ...
    if (businessCycle?.status === 'Completed') {
      // Completed Affiliates may reactivate the current package or upgrade.
      return pkgOrder >= userOrder
    }

    // Active Affiliates may only upgrade to a higher package.
    return pkgOrder > userOrder
  })

  const visibleSmartCustomerPackages = smartCustomerPackages.filter((pkg) => {
    const currentPackage = String(userProfile?.packageLevel || 'None')
      .trim()
      .toLowerCase()
    const currentSmartOrder = smartCustomerOrder[currentPackage] || 0
    const targetSmartOrder = smartCustomerOrder[pkg.name.toLowerCase()] || 0

    if (currentSmartOrder === 0) return true
    return targetSmartOrder > currentSmartOrder
  })
  ...
  if (hasPackage) {
    if (isCycleCompleted) {
      ...
      subtitleText =
        'Your Business Cycle is completed. Reactivate your current package or upgrade to a higher package to continue receiving commissions, privileges, and business benefits.'
    } else {
      badgeText = 'Upgrade Mode'
      titleText = 'Upgrade Account Tier'
      subtitleText =
        'Your Business Cycle is active. You may upgrade to a higher package to unlock greater earning capacity and business benefits.'
    }
  }
      ...
      <div className='p-6 max-w-7xl mx-auto w-full flex items-center justify-between border-b border-zinc-900/60'>
        ...
        <div className='flex items-center gap-3'>
          ...
          <span className='px-2.5 py-1 rounded-full text-[10px] font-bold bg-zinc-900 text-gold border border-gold/20 font-mono'>
            {fetchingWallet
              ? '...'
              : `${userWallet?.chosenWalletBalance?.toFixed(2) || '0.00'} CC`}
          </span>
        </div>
      </div>
          ...
          {successData ? (
            ...
            >
              ...
              {successData.path === 'Affiliate Business' ? (
                <>
                  <span className='text-[10px] font-bold font-mono text-emerald-400 uppercase tracking-[0.25em] block mb-2'>
                    {successData.activationAction === 'INITIAL_ACTIVATION'
                      ? 'Activation Successful'
                      : successData.activationAction ===
                  ...
                  <h2 className='text-2xl font-black text-white uppercase tracking-tight mb-4'>
                    {successData.activationAction === 'INITIAL_ACTIVATION'
                      ...
                      : successData.activationAction ===
                          'BUSINESS_CYCLE_REACTIVATION'
                        ? 'Your Business Cycle has been reactivated.'
                        : 'Your Affiliate package has been upgraded.'}
                  </h2>
                ...
                <>
                  ...
                  <h2 className='text-2xl font-black text-white uppercase tracking-tight mb-4'>
                    Your Smart Customer package is active.
                  </h2>
                  <p className='text-zinc-400 text-xs leading-relaxed max-w-sm mx-auto mb-8 font-medium'>
                    Your{' '}
                    <strong className='text-white'>
                    ...
                    package was purchased securely using your Chosen Wallet.
                  </p>
                </>
              )}
              ...
              <div className='bg-zinc-950/60 rounded-2xl border border-zinc-900/80 p-5 mb-8 text-left space-y-3.5'>
                ...
                <div className='flex justify-between items-center text-xs'>
                  ...
                  <span className='font-extrabold text-gold text-sm'>
                    ₱{successData.amountPHP.toLocaleString()}
                  </span>
                </div>
                <div className='h-px bg-zinc-900' />
                <div className='flex justify-between items-center gap-4 text-xs'>
                  ...
                  <span className='font-extrabold text-white text-right'>
                    {successData.activationAction.replaceAll('_', ' ')}
                  </span>
                </div>
                <div className='h-px bg-zinc-900' />
                <div className='flex justify-between items-center gap-4 text-xs'>
                  <span className='text-zinc-500 font-mono'>REFERENCE</span>
                  <span className='font-mono text-[10px] text-cyan-300 text-right break-all'>
                    {successData.activationEventId}
                  </span>
                </div>
              </div>

              <div className='mb-4 rounded-2xl border border-cyan-500/15 bg-cyan-500/5 p-3.5'>
                <div className='flex items-center justify-center gap-2 text-[10px] font-bold uppercase tracking-wider text-cyan-300'>
                  <RefreshCw
                    className={`w-3.5 h-3.5 ${
                      successRedirecting ? 'animate-spin' : ''
                    }`}
                  />
                  {successRedirecting
                    ? 'Refreshing your dashboard...'
                    : `Returning to the main dashboard in ${successRedirectSeconds} seconds`}
                </div>
              </div>
              ...
              <div className='flex gap-4'>
                <button
                  type='button'
                  onClick={returnToDashboardAfterSuccess}
                  disabled={successRedirecting}
                  className='flex-1 py-3 bg-gradient-to-r from-amber-500 to-[#D4AF37] hover:from-amber-400 text-black font-bold text-xs uppercase tracking-widest rounded-xl transition-all cursor-pointer shadow-lg font-extrabold disabled:opacity-60 disabled:cursor-wait flex items-center justify-center gap-2'
                >
                  {successRedirecting ? (
                    <>
            ...
            >
              ...
              <div className='text-center max-w-3xl mx-auto flex flex-col items-center'>
                ...
                {isAffiliateFlow ? (
                  <>
                    <span
                      className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold border mb-4 uppercase tracking-widest font-mono
                      ${
                        isCycleCompleted
                    ...
                    {hasPackage ? (
                      <span className='block text-xs font-semibold text-zinc-400 mb-4 tracking-wide'>
                        ...
                        <span className='text-gold font-bold uppercase font-mono'>
                          {userProfile.packageLevel} Package
                        </span>
                        {businessCycle && (
                          <span
                            className={`ml-2 px-2 py-0.5 rounded text-[9px] font-bold uppercase ${isCycleCompleted ? 'bg-rose-500/10 text-rose-400 border border-rose-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'}`}
                          >
                  ...
                  <>
                    ...
                    <p className='text-zinc-400 text-xs sm:text-sm mt-3 max-w-2xl font-semibold leading-relaxed'>
                      Choose a customer package and enjoy exclusive discounts,
                      rebates, and premium wellness benefits.
                    </p>
                  </>
                )}
              </div>
                        ...
                        >
                          ...
                          <div className='flex-1 flex flex-col'>
                            ...
                            <div className='flex items-center justify-between mb-4'>
                              <span
                                className={`inline-flex items-center px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest font-mono border ${style.badgeBg}`}
                              >
                                {pkg.name} Tier
                              </span>
                            </div>
                            ...
                            <div className='mb-5 text-left'>
                              <div className='flex items-baseline gap-1.5'>
                                ...
                                <span
                                  className={`text-sm font-black uppercase tracking-wider ${style.accentText}`}
                                >
                                  CC
                                </span>
                              </div>
                            ...
                            <div className='border-t border-zinc-900/80 pt-5 mb-6 text-left flex-1 flex flex-col'>
                              <span className='block uppercase font-mono text-[9px] text-zinc-500 tracking-wider mb-3'>
                                Core Membership Privileges
                              </span>
                              <div className='space-y-3 flex-1'>
                                {pkg.benefits.map((benefit, bIdx) => (
                                  <div
                                    key={bIdx}
                                    className='flex items-start gap-2.5 text-xs text-zinc-300 font-medium'
                                  >
                          ...
                          <div className='mt-auto space-y-4'>
                            <div className='bg-zinc-950/80 border border-zinc-900 rounded-2xl py-3 px-4 text-center'>
                              <div className='flex items-center justify-between text-xs'>
                                <span className='text-zinc-500 font-mono tracking-wider text-[10px] uppercase'>
                                  Earnings Limit (2.5x)
                                </span>
                                <span
                                  className={`text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-wider bg-zinc-900 border border-zinc-800 ${style.accentText}`}
                                >
                                  Active Cap
                                </span>
                              </div>
              ...
              visibleSmartCustomerPackages.length > 0 ? (
                <div className='grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto'>
                  {visibleSmartCustomerPackages.map((pkg) => (
                    <motion.div
                      key={pkg.name}
                      whileHover={{ y: -3 }}
                    ...
                    >
                      ...
                      <div>
                        <span className='block text-[9px] uppercase tracking-widest text-zinc-500 font-black mb-1 font-mono'>
                          Customer Tier
                        </span>
                        <h3 className='text-xl font-black text-white uppercase group-hover:text-emerald-400 transition-colors'>
                          {pkg.name}
                        </h3>

                        {/* CC and Price Tag */}
                        <div className='bg-zinc-950/60 py-3.5 rounded-xl border border-zinc-900 my-4'>
                          <span className='block text-base text-emerald-400 font-black'>
                            {pkg.cc} CC
                          </span>
                        ...
                        <div className='border-t border-zinc-900/80 pt-4 mb-6 text-left'>
                          <span className='block uppercase font-mono text-[9px] text-zinc-500 mb-2.5'>
                            Key Benefits
                          </span>
                ...
                <div className='max-w-xl mx-auto rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-6 text-center'>
                  <p className='text-sm font-bold text-emerald-300'>
                    You already have the highest Smart Customer package.
                  </p>
                  <p className='mt-2 text-xs text-zinc-400'>
                    Choose Affiliate Business to continue to a business package.
                  </p>
                </div>
              )}
            </motion.div>
          ...
          <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md'>
            ...
            >
              <div className='flex items-center gap-3.5 mb-4 border-b border-zinc-900/60 pb-3'>
                <div
                  className={`w-11 h-11 rounded-xl flex items-center justify-center ${isAffiliateFlow ? 'bg-amber-500/10 border border-amber-500/20 text-[#D4AF37]' : 'bg-emerald-500/10 border border-emerald-500/20 text-emerald-400'}`}
                >
                  {isAffiliateFlow ? (
                ...
                <div>
                  ...
                  <span className='block text-[9px] text-zinc-500 font-mono font-bold uppercase tracking-wider'>
                    {isAffiliateFlow
                      ? 'Affiliate Track Onboarding'
                      : 'Smart Customer Procurement'}
                  </span>
                </div>
              </div>
              ...
              <div className='space-y-4 mb-6'>
                <div className='bg-zinc-950/80 rounded-2xl border border-zinc-900/60 p-4 space-y-3'>
                  <div className='flex justify-between items-center text-xs'>
                    <span className='text-zinc-500 font-mono uppercase text-[9px] tracking-wider'>
                      Membership
                    </span>
                    <span className='font-extrabold text-white uppercase'>
                      {selectedPackageToConfirm.name} Package
                    </span>
                  </div>
                  <div className='h-px bg-zinc-900' />
                  <div className='flex justify-between items-center text-xs'>
                    ...
                    <span className='font-black text-white font-mono'>
                      {selectedPackageToConfirm.cc} CC
                    </span>
                  </div>
                  <div className='h-px bg-zinc-900' />
                  <div className='flex justify-between items-center text-xs'>
                    ...
                    <span className='font-black text-gold text-sm'>
                      ₱{selectedPackageToConfirm.php.toLocaleString()}
                    </span>
                  </div>
                    ...
                    <>
                      ...
                      <div className='flex justify-between items-center text-xs'>
                        ...
                        <span className='font-black text-emerald-400 font-mono'>
                          {selectedPackageToConfirm.cap} CC Max
                        </span>
                      </div>
                    </>
                  )}
                  ...
                  <div className='bg-zinc-950/40 p-4 rounded-xl border border-zinc-900 text-xs space-y-2.5'>
                    ...
                    <div className='flex justify-between items-center text-zinc-400'>
                      ...
                      <span className='text-white font-bold'>
                        {previewData.requiredAmountCC} CC
                      </span>
                    </div>
                    {previewData.hasSufficientBalance ? (
                      <div className='flex justify-between items-center text-zinc-400 pt-1.5 border-t border-zinc-900'>
                        ...
                        <span className='text-emerald-400 font-bold'>
                          {previewData.remainingBalanceAfterPurchaseCC.toFixed(
                            2,
                          )}{' '}
                          CC
                        </span>
                      ...
                      <div className='space-y-2 pt-2 border-t border-zinc-900'>
                        ...
                        <div className='text-[10px] text-zinc-400 bg-amber-500/10 border border-amber-500/20 p-2.5 rounded-lg leading-relaxed text-left'>
                          ⚠️ <strong>Insufficient balance.</strong> You need{' '}
                          <strong>
                            {previewData.shortageCC.toFixed(2)} CC
                          </strong>{' '}
                          more. Click <strong>Continue to Cash-In</strong> to
                          fund your wallet and complete your purchase.
                        </div>
                      </div>
                    )}
                  </div>
                ...
                <div className='border-t border-zinc-900/60 pt-4'>
                  <span className='block uppercase font-mono text-[9px] text-zinc-500 tracking-wider mb-2'>
                    Benefits Summary
                  </span>
