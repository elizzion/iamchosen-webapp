...
import { doc, getDoc, setDoc, updateDoc, collection, getDocs, query, where, writeBatch, limit } from 'firebase/firestore';
import { UserProfile, Wallet as WalletType, BusinessCycle, Notification } from '../types';
...
export default function AffiliateDashboard({ userProfile, onLogout, onNavigate }: AffiliateDashboardProps) {
  ...
  const [activityFilter, setActivityFilter] = useState<'ALL' | 'TRANSACTIONS' | 'COMMISSIONS' | 'REFERRALS'>('ALL');
  ...
  useEffect(() => {
    ...
    if (pendingView) {
      if (['team', 'orders', 'commissions', 'marketing', 'academy', 'support'].includes(pendingView)) {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      txList.sort((a: any, b: any) => new Date(b.timestamp || b.createdAt).getTime() - new Date(a.timestamp || a.createdAt).getTime());
    ...
    try {
      ...
      console.error("Error loading dashboard details (Step 5: Cash-in History):", err);
    }

    // 6. Fetch Commissions Summary
    try {
      const summary = await CommissionService.getCommissionSummary(userProfile.uid);
      setCommissionSummary(summary);
    } catch (err) {
      console.error("Error loading dashboard details (Step 6: Commission Summary):", err);
    }
    ...
    try {
      ...
      const q1 = query(collection(db, 'users'), where('referredBy', '==', userProfile.uid));
      const s1 = await getDocs(q1);
      s1.docs.forEach(d => { refs[d.id] = d.data(); });

      // Query 2: sponsorUid == userProfile.uid
      const q2 = query(collection(db, 'users'), where('sponsorUid', '==', userProfile.uid));
      const s2 = await getDocs(q2);
      s2.docs.forEach(d => { refs[d.id] = d.data(); });

      // Query 3: referredBy == userProfile.sponsorCode (if sponsorCode is defined)
      if (userProfile.sponsorCode) {
        const q3 = query(collection(db, 'users'), where('referredBy', '==', userProfile.sponsorCode));
        const s3 = await getDocs(q3);
        s3.docs.forEach(d => { refs[d.id] = d.data(); });
      }
    ...
    try {
      ...
      console.error("Error loading dashboard details (Step 8: CC business_rules):", err);
      setDirectReferralRate(4);
    }
  ...
  const handleCashoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCashoutSuccess(`Successfully requested cashout of ${cashoutAmountCC} CC! Net: ₱${res.netPhp.toLocaleString()} scheduled for release this Friday.`);
      setAccountName('');
      setTimeout(() => {
  ...
  const handleCashinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setCashinSuccess(`Cash-In request for ₱${cashinAmountPhp.toLocaleString()} submitted successfully! We are validating your receipt.`);
      setCashinReference('');
      setReceiptFile(null);
      setTimeout(() => {
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await WalletService.createTransferRequest(userProfile.uid, userProfile.email, wallet, {
        recipientEmail: transferRecipient,
        amountCC: transferAmount,
        memberId: userProfile.memberId
      });

      setTransferSuccess(`Successfully transferred ${transferAmount} CC to ${transferRecipient}!`);
  ...
  const handleUpgradeSubmit = async (e: React.FormEvent) => {
    ...
    if (targetValue <= currentValue) {
      setUpgradeError(`You cannot upgrade to ${selectedUpgradeLevel} because it is equal to or lower than your current package tier (${currentLevel}).`);
      return;
    }
    ...
    if (!wallet || wallet.chosenWalletBalance < costCC) {
      setUpgradeError(`Insufficient Chosen Wallet balance. You need ${costCC} CC to upgrade to ${selectedUpgradeLevel}, but your Chosen Wallet balance is only ${wallet?.chosenWalletBalance || 0} CC.`);
      return;
    }
    ...
    try {
      ...
      setUpgradeSuccess(`Congratulations! Successfully upgraded your package level to ${selectedUpgradeLevel}!`);
      setTimeout(() => {
  ...
  const handleAskAICoach = async (promptText?: string) => {
    ...
    try {
      ...
      setAICoachHistory(prev => [...prev, { sender: 'ai' as const, text: response }]);
    } catch (e) {
      console.error(e);
      setAICoachHistory(prev => [...prev, { sender: 'ai' as const, text: 'I apologize, I am unable to connect to the model. Let me try again.' }]);
    } finally {
  ...
  const simulatorTiers = [
    ...
    { name: 'Regional Distributor', fallbackCC: undefined, isDistributor: true },
  ];
  ...
  const getCycleProgressPercentage = (): number => {
    ...
    if (businessCycle) {
      ...
      } else if (typeof (businessCycle as any).currentQualifiedEarningsPHP === 'number') {
        current = (businessCycle as any).currentQualifiedEarningsPHP / cashInRateVal;
      }
      ...
      } else if (typeof (userProfile as any).currentQualifiedEarningsPHP === 'number') {
        current = (userProfile as any).currentQualifiedEarningsPHP / cashInRateVal;
      }
    ...
    if (typeof current !== 'number' || isNaN(current) || typeof cap !== 'number' || isNaN(cap) || cap <= 0) {
      return 0;
    }
  ...
  const activeAffiliatePackages = ["Bronze", "Silver", "Gold", "Platinum", "Diamond", "City Distributor", "Regional Distributor"];
  ...
  const activeDirectAffiliates = (downlineList || []).filter(member => {
    ...
    const hasActivePackage = activeAffiliatePackages.includes(member.packageLevel);
    const isActive = member.status === "Active";
    return isAffiliate && hasActivePackage && isActive;
  });

  // 1. Personal Sales in CC (convert from PHP if only PHP exists)
  const personalSalesCC = activeDirectAffiliates.reduce((sum, member) => sum + getPackageCCValue(member.packageLevel || 'Bronze'), 0);

  // 2. Group Volume in CC
  const groupVolumeCC = activeDirectAffiliates.reduce((sum, member) => sum + 50, 0); // Each active member contributes 50 CC unilevel volume

  // 3. Personal Volume in CC
  const rawPersonalVolume = userProfile && (userProfile as any).personalVolumeCC !== undefined 
    ...
    : (userProfile && (userProfile as any).personalVolumePHP !== undefined
      ...
      : 50.00);
  const personalVolumeCC = typeof rawPersonalVolume === 'number' && !isNaN(rawPersonalVolume) ? rawPersonalVolume : 0;
  ...
  if (commissionSummary && typeof commissionSummary.grandTotal === 'number' && commissionSummary.grandTotal > 0) {
    rawMonthlyEarnings = commissionSummary.grandTotal;
  } else if (userProfile && (userProfile as any).monthlyEarningsCC !== undefined) {
    rawMonthlyEarnings = (userProfile as any).monthlyEarningsCC;
  } else if (userProfile && (userProfile as any).monthlyEarningsPHP !== undefined) {
    rawMonthlyEarnings = (userProfile as any).monthlyEarningsPHP / cashInRate;
  } else if (commissionSummary && typeof commissionSummary.grandTotal === 'number') {
    rawMonthlyEarnings = commissionSummary.grandTotal;
  }
  const monthlyEarningsCC = typeof rawMonthlyEarnings === 'number' && !isNaN(rawMonthlyEarnings) ? rawMonthlyEarnings : 0;
        ...
        <div className="bg-zinc-950 border border-zinc-800/80 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex justify-between items-start mb-5">
            <div>
              <h3 className="font-extrabold text-sm text-white uppercase tracking-tight flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-amber-500 animate-pulse" /> Active Business Plan Cycle
              </h3>
            </div>
            {businessCycle && (
              <span className={`text-[9px] font-mono px-2.5 py-1 rounded-full uppercase font-bold border ${
                businessCycle.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/15' : 'bg-red-500/10 text-red-400 border-red-500/15'
              }`}>
          ...
          {businessCycle ? (
            <div className="space-y-5">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-xs font-mono">
                  <span className="text-zinc-400 font-semibold">Qualified Earnings Balance:</span>
                  <span className="text-zinc-200 font-bold">{businessCycle.currentQualifiedEarningsCC} / {businessCycle.earningsCapCC} CC</span>
                </div>
                <div className="w-full bg-zinc-900 h-2.5 rounded-full overflow-hidden border border-zinc-800">
                  <div 
                    className="gold-gradient h-full transition-all duration-500" 
                    style={{ width: `${Math.min(100, (businessCycle.currentQualifiedEarningsCC / businessCycle.earningsCapCC) * 100)}%` }}
                  />
                </div>
                <div className="flex justify-between text-[10px] text-zinc-500 uppercase font-mono">
                  <span>Cycle Consumption Progress</span>
                  <span className="text-amber-500 font-bold">{businessCycle.remainingCapacityCC} CC capacity remaining</span>
                </div>
              </div>
            </div>
          ) : (
            <div className="text-center py-8">
              ...
              <h4 className="font-extrabold text-white text-sm uppercase tracking-tight">Business Cycle not initialized</h4>
              <p className="text-zinc-500 text-xs mt-2 max-w-sm mx-auto">
                You do not have an active package tier cycle on file. Please purchase a Bronze, Silver, Gold, Platinum, or Diamond package to initialize your earnings ledger.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="bg-zinc-950 border border-zinc-800/80 rounded-3xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-5">
            <div>
              ...
              <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono mt-1">Audit Ledger & Network Milestones</p>
            </div>
            ...
            <div className="flex gap-1.5 bg-zinc-900/60 p-1 rounded-xl border border-zinc-850">
              <button 
                ...
                className={`text-[9px] font-bold uppercase px-2.5 py-1 rounded-lg transition-colors cursor-pointer ${
                  activityFilter === 'ALL' ? 'bg-zinc-850 text-white border border-zinc-800' : 'bg-transparent text-zinc-500 hover:text-zinc-300'
                }`}
              >
                All Activities
              </button>
              <button 
                ...
                className={`text-[9px] font-bold uppercase px-2.5 py-1 rounded-lg transition-colors cursor-pointer ${
                  activityFilter === 'TRANSACTIONS' ? 'bg-zinc-850 text-white border border-zinc-800' : 'bg-transparent text-zinc-500 hover:text-zinc-300'
                }`}
              >
                Financial Ledger
              </button>
          ...
          {filteredActivities.length === 0 ? (
            ...
            <div className="space-y-2.5 max-h-[350px] overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-zinc-800 scrollbar-track-transparent">
              {filteredActivities.map((item, i) => (
                <div key={item.id || i} className="bg-zinc-900/30 hover:bg-zinc-900/55 border border-zinc-850 p-3.5 rounded-2xl flex justify-between items-center text-xs transition-colors">
                  <div>
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="font-bold text-white text-[11px] leading-tight">{item.title}</span>
                    </div>
                    <span className="block text-[8px] text-zinc-500 font-mono mt-1 uppercase tracking-wider">
                      {item.date} • {item.subtext}
                    </span>
                  </div>
                  <span className={`font-mono font-black text-xs ml-2 shrink-0 ${
           ...
           <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
              ...
              <div className="flex items-center space-x-3">
                 ...
                 <div className="flex items-center space-x-2 bg-zinc-900/80 border border-zinc-800/80 rounded-xl px-3 py-1.5">
                    <div className={`w-6 h-6 rounded-full ${getPackageConfig(userProfile.packageLevel).bgClass} border ${getPackageConfig(userProfile.packageLevel).accentClass} flex items-center justify-center text-[10px] font-bold font-mono`}>
                      {userProfile.fullName ? userProfile.fullName.substring(0, 2).toUpperCase() : 'IA'}
                    </div>
                    <div className="text-left hidden sm:block">
                      <span className="block text-[10px] font-black text-white leading-tight">{userProfile.fullName}</span>
                      <span className={`block text-[8px] ${getPackageConfig(userProfile.packageLevel).accentClass.split(' ')[0]} uppercase font-bold tracking-wider`}>
              ...
              <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-6 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative overflow-hidden">
                ...
                <div>
                  <h3 className="font-extrabold text-sm uppercase tracking-wider text-red-400 flex items-center gap-2">
                    <ShieldAlert className="w-4 h-4 animate-pulse text-red-500" /> Business Cycle Completed
                  </h3>
                  <p className="text-xs text-zinc-300 mt-1 max-w-2xl leading-relaxed">
                    Your Business Cycle is completed. Reactivate or upgrade to continue earning commissions.
                  </p>
                </div>
                <div className="flex gap-2.5 shrink-0 w-full md:w-auto mt-2 md:mt-0">
                   <button
                     onClick={() => {
                       window.history.pushState({}, '', '/package-selection?type=affiliate-business&action=reactivate');
                   ...
                   <button
                     onClick={() => {
                       window.history.pushState({}, '', '/package-selection?type=affiliate-business&action=upgrade');
                       onNavigate('package-selection');
                     }}
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono mt-0.5">
                      Core Credits • Direct Ledger & Earnings Balance
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
                  <div className="flex items-center gap-4 text-sm sm:text-xs font-mono pr-2">
                    <div>
                      <span className="text-zinc-400 uppercase text-[10px] sm:text-[8px] block font-bold">Chosen Balance</span>
                      <span className="text-white font-black text-base sm:text-sm">{wallet ? wallet.chosenWalletBalance.toFixed(2) : '0.00'} CC</span>
                    </div>
                    <div className="w-px h-8 sm:h-6 bg-zinc-800" />
                    <div>
                      <span className="text-zinc-400 uppercase text-[10px] sm:text-[8px] block font-bold">Commissions</span>
                      <span className="text-amber-500 font-black text-base sm:text-sm">{wallet ? wallet.commissionWalletBalance.toFixed(2) : '0.00'} CC</span>
                    </div>
                  </div>
                  ...
                  <div className="w-7 h-7 bg-zinc-900 border border-zinc-800 rounded-lg flex items-center justify-center text-zinc-400 shrink-0">
                    {isWalletExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </div>
                </div>
              </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-4 shadow-md relative overflow-hidden group">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-mono">Commission</span>
                      ...
                      <span className="block text-[8px] text-zinc-500 uppercase font-bold mt-2">Earnings Balance</span>
                      <button
                        onClick={() => setShowUpgradeModal(true)}
                        className="mt-3 w-full bg-zinc-900 hover:bg-zinc-850 border border-zinc-800 hover:border-gold/40 text-white hover:text-gold font-bold text-[9px] py-1.5 rounded-lg transition-all cursor-pointer flex items-center justify-center gap-1"
                      >
                        <Sparkles className="w-3 h-3" /> Upgrade Package
                      </button>
                    </div>
                    ...
                    <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-4 shadow-md relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-mono">Marketing Support</span>
                      ...
                      <span className="block text-[8px] text-zinc-500 uppercase font-bold mt-2">Co-op Funds</span>
                    ...
                    <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-4 shadow-md relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-mono">Reward Wallet</span>
                      ...
                      <span className="block text-[8px] text-zinc-500 uppercase font-bold mt-2">Gift & Incentives</span>
                      <div className="mt-3 h-6 flex items-center justify-center text-[8px] text-zinc-500 uppercase tracking-wider border border-zinc-900 bg-zinc-950 rounded-lg">
                        Non-Withdrawable
                      </div>
                    </div>
                    ...
                    <div className="bg-zinc-950 border border-zinc-850 rounded-2xl p-4 shadow-md relative overflow-hidden">
                      ...
                      <div className="flex justify-between items-start mb-2">
                        <span className="text-[9px] text-zinc-500 uppercase tracking-widest font-mono">Cash Wallet</span>
                        <div className="w-5 h-5 bg-teal-500/10 rounded-md flex items-center justify-center border border-teal-500/25 text-teal-400">
                          <TrendingUp className="w-3 h-3" />
                        </div>
                      </div>
                      <div className="text-base font-black tracking-tight text-white mb-0.5">
                        {wallet?.cashWalletStatus || 'Active'}
                      </div>
                      <div className="text-[9px] text-zinc-400 font-mono">
                        Channel: GCash / Maya / Bank
                      </div>
                      <span className="block text-[8px] text-zinc-500 uppercase font-bold mt-2">Cash-Out Gateway</span>
                      <button
        ...
        <div className="bg-zinc-950 border border-zinc-800/80 rounded-3xl p-6 shadow-xl relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-extrabold text-xs text-white uppercase tracking-tight">Need help growing your business?</h4>
                <p className="text-zinc-400 text-xs mt-1 leading-relaxed font-light">
                  Ask your AI Business Coach how to get your first customers, invite members, and grow from Bronze to Silver.
                </p>
              </div>
            </div>
            <button
      ...
      {showTransferModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm animate-fadeIn">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative">
            ...
            <h3 className="text-lg font-black uppercase tracking-tight mb-1 text-white">P2P Credit Transfer</h3>
            ...
            {transferError && <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-3 rounded-xl text-xs mb-4">{transferError}</div>}
            {transferSuccess && <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-3 rounded-xl text-xs mb-4">{transferSuccess}</div>}

            <form onSubmit={handleTransferSubmit} className="space-y-4">
              <div>
                <label className="block text-[10px] uppercase tracking-wider text-zinc-400 font-bold mb-1.5">Recipient Account Email</label>
                <input
                  type="email"
                  required
              ...
              <div>
                <label className="block text-[10px] uppercase tracking-wider text-zinc-400 font-bold mb-1.5">Amount in Credits (CC)</label>
                <input
                  type="number"
                  required
                  min="1"
                ...
                />
                <span className="block text-[9px] text-zinc-500 mt-1">A flat fee of 0.50 CC is charged per transaction.</span>
              </div>
          ...
          <div className="w-full max-w-xl bg-zinc-950 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative h-[500px] flex flex-col justify-between">
            ...
            <div className="flex justify-between items-center pb-3 border-b border-zinc-800/80">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-sm font-black uppercase tracking-tight text-white leading-none">AI Business Coach</h3>
                  <span className="text-[8px] text-zinc-500 uppercase tracking-wider font-mono">Bronze Growth Companion</span>
                </div>
              </div>
              <button onClick={() => setShowAICoachModal(false)} className="text-zinc-500 hover:text-white text-xs uppercase font-bold px-2 py-1">Close</button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto my-4 space-y-4 pr-1 text-xs">
              {aiCoachHistory.length === 0 ? (
                <div className="text-center py-10 space-y-4">
                  <p className="text-zinc-500">Welcome to your AI Business Coach! Select a suggested topic below to begin planning your Bronze strategy:</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-md mx-auto">
                    <button onClick={() => handleAskAICoach('How do I get my first 5 customers in IAM CHOSEN?')} className="bg-zinc-900 hover:bg-zinc-850 p-3 rounded-xl border border-zinc-850 hover:border-teal-500/30 text-left text-zinc-300 transition-all font-medium">
                      🎯 How to get my first 5 customers?
                    </button>
                    <button onClick={() => handleAskAICoach('How can I invite members to register using my sponsor link?')} className="bg-zinc-900 hover:bg-zinc-850 p-3 rounded-xl border border-zinc-850 hover:border-teal-500/30 text-left text-zinc-300 transition-all font-medium">
                      👥 How to invite active members?
                    </button>
                    <button onClick={() => handleAskAICoach('What is the fastest strategy to upgrade from Bronze to Silver?')} className="bg-zinc-900 hover:bg-zinc-850 p-3 rounded-xl border border-zinc-850 hover:border-teal-500/30 text-left text-zinc-300 transition-all font-medium">
                      🚀 Strategy: Upgrade to Silver
                    </button>
                    <button onClick={() => handleAskAICoach('How do unilevel commission cycles and safety caps work?')} className="bg-zinc-900 hover:bg-zinc-850 p-3 rounded-xl border border-zinc-850 hover:border-teal-500/30 text-left text-zinc-300 transition-all font-medium">
                ...
                aiCoachHistory.map((m, i) => (
                  <div key={i} className={`flex ${m.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                    <div className={`p-4 rounded-2xl max-w-[85%] leading-relaxed whitespace-pre-line border ${
                      m.sender === 'user' 
              ...
              {aiCoachLoading && (
                <div className="flex justify-start">
                  <div className="p-4 bg-zinc-900/40 border border-zinc-850 rounded-2xl flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">Analyzing Business Guidelines...</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-6 shadow-2xl relative">
            ...
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-sm font-black uppercase tracking-tight text-white">
                ...
                {activeActionModal === 'commissions' && 'Unilevel Commissions Breakdown'}
                {activeActionModal === 'marketing' && 'Marketing Co-op Funding'}
                {activeActionModal === 'academy' && 'Ecosystem Training Academy'}
                {activeActionModal === 'support' && 'Sponsor Help & Support'}
                {activeActionModal === 'qrcode' && 'My Sponsor QR Code'}
              </h3>
              <button onClick={() => setActiveActionModal(null)} className="text-zinc-500 hover:text-white text-xs uppercase font-extrabold">Close</button>
            </div>
            ...
            <div className="text-xs text-zinc-400 space-y-4">
              ...
              {activeActionModal === 'shop' && (
                <div className="space-y-4">
                  <div className="p-4 bg-amber-500/5 border border-amber-500/10 text-amber-500 rounded-xl">
                    <span className="block font-bold uppercase tracking-wider text-[11px] mb-1">Commerce Integration (Phase 3)</span>
                    <p className="font-light leading-relaxed">
                      The official corporate products commerce store catalog is scheduled for rollout in Sprint 4.
                    </p>
                  </div>
                  <div className="bg-zinc-900/40 p-4 rounded-xl border border-zinc-850 space-y-2">
                    <span className="block font-bold text-white uppercase tracking-wider text-[10px]">Upcoming Products:</span>
                    <div className="flex justify-between font-mono"><span>Pure Barley (1 Box)</span><span className="text-gold font-bold">16 CC</span></div>
                    <div className="flex justify-between font-mono"><span>Iced Barley Coffee (1 Box)</span><span className="text-gold font-bold">16 CC</span></div>
                    <div className="flex justify-between font-mono"><span>Barley Latte (1 Box)</span><span className="text-gold font-bold">16 CC</span></div>
                  </div>
                </div>
              )}
              ...
              {activeActionModal === 'orders' && (
                <div className="space-y-3">
                  ...
                  <div className="bg-zinc-900/30 p-4 rounded-2xl border border-zinc-850/60 text-[11px]">
                    <span className="block font-bold text-zinc-300 uppercase mb-1">Commerce Ingress</span>
                    <p className="leading-relaxed">All package activations and repurchase records are tracked directly on your active business plan cycle ledger.</p>
                  </div>
                </div>
              )}
              ...
              {activeActionModal === 'team' && (
                <div className="space-y-4">
                  <div className="bg-zinc-900/40 p-4 rounded-2xl border border-zinc-850 text-center">
                    <span className="block text-[8px] text-zinc-500 uppercase font-mono">Total Direct Referrals</span>
                    <span className="text-2xl font-black text-white">{downlineList.length} Active Nodes</span>
                  </div>
                  
                  <div className="space-y-2 max-h-[220px] overflow-y-auto">
                    {downlineList.length === 0 ? (
                      <p className="text-center text-zinc-500 py-6">You have no active downline members yet. Share your sponsor link to register your first partner!</p>
                    ) : (
                      downlineList.map((m, i) => (
                        <div key={i} className="bg-zinc-900/50 p-3 rounded-xl border border-zinc-850 flex justify-between items-center">
                          <div>
                            <span className="block font-bold text-white">{m.fullName}</span>
                            <span className="block text-[8px] font-mono text-zinc-500 uppercase mt-0.5">{m.memberId} • Pack: {m.packageLevel || 'Bronze'}</span>
                          </div>
                          <span className="text-[10px] font-bold text-emerald-400 uppercase bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/15">Active</span>
                        </div>
                      ))
                    )}
                  </div>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-zinc-850">
                      <span className="block text-[8px] text-zinc-500 uppercase font-mono mb-1">Direct Referrals</span>
                      <span className="text-sm font-black text-white font-mono">{commissionSummary?.totalDirectReferral.toFixed(2)} CC</span>
                    </div>
                    <div className="bg-zinc-900/40 p-4 rounded-xl border border-zinc-850">
                      <span className="block text-[8px] text-zinc-500 uppercase font-mono mb-1">Unilevel Bonus</span>
                      <span className="text-sm font-black text-white font-mono">{commissionSummary?.totalUnilevel.toFixed(2)} CC</span>
                    </div>
                  </div>
                  <div className="p-4 bg-zinc-900/30 border border-zinc-850 rounded-2xl">
                    <span className="block font-bold text-zinc-300 uppercase text-[10px] mb-1">Ecosystem Matching Guidelines</span>
                    <p className="leading-relaxed text-[11px]">{getPackageConfig(userProfile.packageLevel).displayName}s accumulate 4% on direct unilevel sales, subject to a total lifecycle headroom of {getPackageConfig(userProfile.packageLevel).cycleMax} CC.</p>
                  </div>
                </div>
              )}
              ...
              {activeActionModal === 'marketing' && (
                <div className="space-y-3 text-[11px] leading-relaxed">
                  <p>The **IAM CHOSEN Marketing Support Wallet** allocates specialized co-op funding resources to qualified high-tier affiliate leaders for regional offline branding and localized distribution networks.</p>
                  <p>As a <strong className="text-gold">{getPackageConfig(userProfile.packageLevel).displayName}</strong>, your allocation is safely tracked under your locked support ledger, which releases upon upgrading and achieving unilevel milestones.</p>
                </div>
              )}
              ...
              {activeActionModal === 'academy' && (
                <div className="space-y-3">
                  <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-850 flex items-center gap-3">
                    ...
                    <div>
                      <span className="block font-bold text-white">Ecosystem Welcome Guide</span>
                      <span className="block text-[9px] text-zinc-500">Video Lecture • 12 mins</span>
                    </div>
                  </div>
                  <div className="bg-zinc-900/40 p-3 rounded-xl border border-zinc-850 flex items-center gap-3">
                    <div className="w-8 h-8 bg-gold/10 text-gold rounded flex items-center justify-center shrink-0">
                    ...
                    <div>
                      <span className="block font-bold text-white">Understanding Safety Caps</span>
                      <span className="block text-[9px] text-zinc-500">Presentation Slides • PDF</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {activeActionModal === 'support' && (
                <form onSubmit={(e) => { e.preventDefault(); alert("Support ticket created successfully."); setActiveActionModal(null); }} className="space-y-3">
                  <div>
                    <label className="block text-[9px] uppercase tracking-wider text-zinc-400 font-bold mb-1">Subject</label>
                    <input type="text" required placeholder="e.g. Wallet Verification" className="w-full bg-zinc-900 border border-zinc-850 rounded-xl px-3 py-2 text-xs focus:outline-none" />
                  </div>
                  <div>
                    <label className="block text-[9px] uppercase tracking-wider text-zinc-400 font-bold mb-1">Message Description</label>
                    <textarea required placeholder="Describe your inquiry..." className="w-full bg-zinc-900 border border-zinc-850 rounded-xl px-3 py-2 text-xs h-16 resize-none focus:outline-none" />
                  </div>
                  <button type="submit" className="w-full gold-gradient text-black font-extrabold py-2 rounded-xl uppercase text-[10px] tracking-wider transition-all hover:brightness-110">Submit Support Ticket</button>
                </form>
              )}
              ...
              {activeActionModal === 'qrcode' && (
                <div className="text-center py-6 space-y-4">
                  ...
                  <p className="text-[10px] text-zinc-500 uppercase tracking-widest font-mono">Scan to register directly under {userProfile.fullName}</p>
                </div>
              )}
      ...
      {showUpgradeModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-3xl p-6 my-8 shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold uppercase tracking-tight mb-1 text-white gold-text">Upgrade Affiliate Package</h3>
            ...
            <form onSubmit={handleUpgradeSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-3 bg-zinc-900/40 p-4 rounded-2xl border border-zinc-800/60">
                <div>
                  <span className="block text-[8px] text-zinc-500 uppercase tracking-wider font-semibold font-mono">Current Tier</span>
                  <span className="text-sm font-bold text-white block mt-0.5">{userProfile.packageLevel || 'Bronze'}</span>
                </div>
                <div>
                  <span className="block text-[8px] text-zinc-500 uppercase tracking-wider font-semibold font-mono">Usable Balance</span>
              ...
              <div>
                ...
                >
                  ...
                  {(() => {
                    ...
                    const currentVal = valCCMap[userProfile.packageLevel || 'Bronze'] || 0;
              ...
              {selectedUpgradeLevel && (() => {
                ...
                return (
                  <div className="bg-zinc-900/60 border border-zinc-800/40 p-4 rounded-2xl text-xs space-y-3">
                    <span className="block font-bold text-zinc-300 uppercase tracking-widest text-[9px] mb-1">Upgrade Comparison</span>
                    ...
                    <div className="flex justify-between text-zinc-400">
                      ...
                      <span className="font-mono text-zinc-500">{currentVal} CC</span>
                      <span className="font-mono text-emerald-400 font-bold">→ {targetVal} CC</span>
                    </div>

                    <div className="flex justify-between text-zinc-400">
                      <span>Earnings Cap:</span>
                      <span className="font-mono text-zinc-500">{currentCap} CC</span>
                      <span className="font-mono text-cyan-400 font-bold">→ {targetCap} CC</span>
                    </div>

                    <div className="border-t border-zinc-800/60 pt-3 flex justify-between items-center text-sm">
                      <span className="font-bold text-zinc-300 uppercase tracking-wider text-[10px]">Upgrade Cost (Difference):</span>
                      <span className="font-black text-gold font-mono text-base">{diffVal.toFixed(2)} CC</span>
                    </div>

                    {isInsufficient && (
                      <div className="mt-2 bg-red-500/10 border border-red-500/20 text-red-400 p-2.5 rounded-xl text-[11px] leading-relaxed">
                        ⚠️ <strong>Insufficient credits.</strong> You need an additional <strong>{(diffVal - balance).toFixed(2)} CC</strong> to complete this upgrade. Please close this modal and click "Cash-In" to top up.
                      </div>
                    )}
                  </div>
                );
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button
                  type="submit"
                  disabled={upgradeLoading || !selectedUpgradeLevel || (() => {
                    const valCCMap: Record<string, number> = {
                      Bronze: 50,
                      Silver: 350,
                      Gold: 1500,
                    ...
                    const diffVal = (valCCMap[selectedUpgradeLevel] || 0) - (valCCMap[userProfile.packageLevel || 'Bronze'] || 0);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-850 rounded-3xl p-8 shadow-2xl relative overflow-hidden">
            ...
            <p className="text-zinc-300 text-sm font-light leading-relaxed mb-6">
              You have reached your current Business Cycle earning capacity. To continue receiving commission earnings, privileges, and business benefits, please reactivate your current package or upgrade to a higher package.
            </p>
      ...
      {showCashoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-6 my-8 shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold uppercase tracking-tight mb-2 text-white">Request Cash-Out</h3>
            ...
            <form onSubmit={handleCashoutSubmit} className="space-y-4">
              <div>
                ...
                <p className="text-[10px] text-zinc-500 mt-1">
                  Available Commission: {wallet ? wallet.commissionWalletBalance.toFixed(2) : '0.00'} CC
                </p>
              </div>
      ...
      {showCashinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn overflow-y-auto">
          <div className="w-full max-w-md bg-zinc-950 border border-zinc-800 rounded-3xl p-6 my-8 shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold uppercase tracking-tight mb-2 text-white gold-text">Request Cash-In / Top Up</h3>
            <p className="text-xs text-zinc-500 uppercase tracking-widest font-mono mb-6">
