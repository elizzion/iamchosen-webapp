...
function isPendingCashInRequest(value: unknown): boolean {
...
export default function CustomerDashboard({
  ...
  const [upgradeNavigationError, setUpgradeNavigationError] = useState<
    string | null
  >(null)

  const handleUpgradeSelect = (
    option: 'smart-customer' | 'affiliate-business',
  ) => {
    ...
    try {
      const activationAction =
        userProfile.packageLevel && userProfile.packageLevel !== 'None'
          ? 'PACKAGE_UPGRADE'
          : 'INITIAL_ACTIVATION'
      const accountPath =
        option === 'affiliate-business' ? 'Affiliate' : 'Smart Customer'

      // Store the non-financial routing intent only. Package price, wallet
      // deduction, commissions, MSA, and Business Cycle values are resolved
      // by the secured backend package engine.
      sessionStorage.setItem('selectedPath', option)
      sessionStorage.setItem('packageAccountPath', accountPath)
      sessionStorage.setItem('packageActivationAction', activationAction)

      // Set search params in the current URL for the package selection route.
      const url = new URL(window.location.href)
      url.searchParams.set('type', option)
      url.searchParams.set(
        'action',
        activationAction === 'PACKAGE_UPGRADE' ? 'upgrade' : 'activate',
      )
      ...
      setIsUpgradeNavigating(false)
      setUpgradeNavigationError(
        'We could not open the package selection page. Please try again.',
      )
    }
  }
  ...
  const startCameraScan = () => {
    ...
    const interval = setInterval(() => {
      ...
      if (current >= 100) {
        ...
        setScanResult(
          `Successfully scanned! Found Product: ${randomProduct.name} (${randomProduct.price} CC)`,
        )
      }
    }, 300)
  }
  ...
  useEffect(() => {
    const handlePackageActivationCompleted = () => {
      void fetchDashboardData()
    }

    window.addEventListener(
      'package_activation_completed',
      handlePackageActivationCompleted,
    )
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const filteredTx = txList.filter(
        (tx) =>
          tx.walletType === 'Chosen Wallet' ||
          tx.walletType === 'Chosen' ||
          tx.walletType === 'Chosen Credits (CC)',
      )
      filteredTx.sort(
        (a, b) =>
  ...
  const handlePurchase = async (product: any) => {
    ...
    if (wallet.chosenWalletBalance < product.price) {
      setPurchaseError(
        `Insufficient Chosen Credits (CC). This product requires ${product.price} CC, but you only have ${wallet.chosenWalletBalance.toFixed(2)} CC.`,
      )
      return
    }
    ...
    try {
      ...
      setPurchaseSuccess(
        `Successfully purchased ${product.name}! Your order has been placed.`,
      )
      fetchDashboardData()
    } catch (e: any) {
  ...
  const handleCashinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!existingPendingSnapshot.empty) {
        const pendingDoc = existingPendingSnapshot.docs[0]
        setCashinHistory((current) => [
          { id: pendingDoc.id, ...pendingDoc.data() },
          ...current,
        ])
        setCashinError(
          'You already have a pending Cash-In request. Wait for it to be approved or rejected before submitting another request.',
      ...
      const computedCC = Number(
        (cashinAmountPhp / ccSettings.cashInRatePHP).toFixed(4),
      )
      ...
      setCashinHistory((current) => [requestData, ...current])
      ...
      setCashinSuccess(
        `Successfully requested cash-in of ₱${Number(cashinAmountPhp).toLocaleString()} (${computedCC} CC)! Please wait for admin approval.`,
      )
      setCashinReference('')
  ...
  const handleQuickAction = (
    actionId: 'shop' | 'orders' | 'cashin' | 'profile' | 'support',
  ) => {
    switch (actionId) {
      case 'shop':
      ...
      case 'cashin':
        ...
        setTimeout(() => {
          document
            .getElementById('cashin-form-section')
            ?.scrollIntoView({ behavior: 'smooth' })
        }, 100)
        break
      case 'profile':
  ...
  const products = [
    {
      ...
      emoji: '🌿',
      description:
        'A concentrated herbal beverage formulated to support daily wellness and healthy living.',
    },
    {
      id: 'prod-latte',
      ...
      emoji: '☕',
      description:
        'A premium coffee blend combining rich flavor with carefully selected herbal extracts.',
    },
    {
      id: 'prod-barley',
      ...
      emoji: '🌾',
      description:
        'A barley grass beverage designed to complement a balanced diet and active lifestyle.',
    },
    {
      id: 'prod-caramel',
      ...
      emoji: '🧊',
      description:
        'A refreshing iced coffee blend with a smooth, premium salted caramel flavor.',
    },
    {
      id: 'prod-choco',
      ...
      emoji: '🍫',
      description:
        'A chocolate-flavored barley beverage that combines great taste and powerful nutrients.',
    },
  ]
  ...
  const pendingCashInRequest =
    cashinHistory.find(isPendingCashInRequest) ?? null
        ...
        <div className='border-t border-zinc-800/80 pt-4 space-y-3.5'>
          <div className='flex items-center space-x-3 px-2'>
            ...
            <div className='truncate'>
              ...
              <p className='text-[9px] text-zinc-500 font-mono tracking-wider truncate uppercase mt-0.5'>
            ...
            {purchaseSuccess && (
              ...
              >
                ...
                <button
                  ...
                  className='text-zinc-400 hover:text-white font-extrabold cursor-pointer'
                >
                  ✕
                </button>
              </motion.div>
                ...
                >
                  ...
                  <div className='grid grid-cols-1 lg:grid-cols-3 gap-6 items-start'>
                    ...
                    <div className='lg:col-span-2 space-y-6'>
                      ...
                      <PromoBanner
                        onCtaClick={() => handleQuickAction('shop')}
                      />
                    </div>
              ...
              {activeMobileTab === 'wallet' && (
                ...
                >
                  ...
                  <div
                    ...
                    className='bg-[#1D1F26] border border-zinc-800 rounded-3xl p-6 space-y-5'
                  >
                    <div>
                      <h3 className='font-extrabold text-sm text-white uppercase tracking-tight flex items-center gap-2'>
                        <ArrowUpRight className='w-4.5 h-4.5 text-gold animate-bounce' />{' '}
                        Top Up Wallet via Cash-In
                      </h3>
                      <p className='text-[10px] text-zinc-500 uppercase tracking-widest font-mono mt-1'>
                        Cash-In Rate: 1 CC = ₱
                        {ccSettings.cashInRatePHP.toFixed(2)} | Secure corporate
                        deposit channels
                      </p>
                    </div>

                    {hasPendingCashInRequest && pendingCashInRequest && (
                      <div className='rounded-2xl border border-amber-500/25 bg-amber-500/10 p-4 text-xs text-amber-300'>
                        ...
                        <p className='mt-1 leading-relaxed text-amber-200/80'>
                          You already have a pending Cash-In request. You can
                          submit a new request only after the current request is
                        ...
                        <div className='mt-3 text-[11px]'>
                          ...
                          <strong className='tabular-nums'>
                            {String(
                              (pendingCashInRequest as any).requestId ||
                                (pendingCashInRequest as any).id ||
                                'Pending request',
                            )}
                          </strong>
                        </div>
                      ...
                      <form onSubmit={handleCashinSubmit} className='space-y-4'>
                        ...
                        <div>
                          ...
                          <div className='relative'>
                            <span className='absolute left-4 top-1/2 -translate-y-1/2 text-zinc-500 text-sm font-mono font-bold'>
                              ₱
                            </span>
                            <input
                              type='number'
                        ...
                        <div className='bg-[#0B0B0F]/80 border border-zinc-800/80 p-4 rounded-2xl text-xs space-y-1.5'>
                          <span className='block font-bold text-zinc-500 uppercase tracking-widest text-[9px]'>
                          ...
                          <div className='flex justify-between font-mono text-[10px]'>
                            ...
                            <span className='text-zinc-300'>
                              1 CC = ₱{ccSettings.cashInRatePHP.toFixed(2)}
                            </span>
                          </div>
                          <div className='flex justify-between border-t border-zinc-800/80 pt-2 mt-1.5 text-xs font-bold'>
                            ...
                            <span className='text-gold font-mono'>
                              {(
                                cashinAmountPhp / ccSettings.cashInRatePHP
                              ).toFixed(4)}{' '}
                              CC
                            </span>
                          </div>
                        ...
                        <div>
                          ...
                          <select
                            value={cashinChannel}
                            onChange={(e: any) =>
                              setCashinChannel(e.target.value)
                            }
                          ...
                          >
                            <option value='GCash'>
                              GCash (Company Account: 0947-5047-299)
                            </option>
                            <option value='Maya'>
                              Maya (Company Account: 0947-5047-299)
                            </option>
                            <option value='Bank'>
                              Bank Transfer (BDO Account:
                              00123-4567-890)00693-0170-398
                            </option>
                          </select>
                        </div>
                        ...
                        <div className='grid grid-cols-2 gap-3.5'>
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setCashinAccountName(e.target.value)
                              }
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setCashinAccountNumber(e.target.value)
                              }
                              className='w-full bg-[#0B0B0F] border border-zinc-800 focus:border-cyan-500 focus:outline-none rounded-xl px-4 py-3 text-xs text-white transition-colors'
                            />
                          </div>
                        ...
                        <div>
                          ...
                          <div
                            ...
                            onClick={() =>
                              document
                                .getElementById(
                                  'customer-cashin-file-upload-re',
                                )
                                ?.click()
                            }
                          >
                              ...
                              <div className='space-y-2'>
                                ...
                                proofOfPaymentUrl
                                  ...
                                  <div className='text-xs font-semibold text-emerald-400 font-mono'>
                                    PDF Receipt Loaded Successfully
                                  </div>
                                ) : (
                                  <img
                                ...
                                <p className='text-[10px] text-zinc-500 font-bold uppercase tracking-wider'>
                                  Tap region to replace file
                                </p>
                              </div>
                            ) : (
                              <div className='py-2 space-y-2'>
                                <div className='text-zinc-300 font-extrabold text-xs'>
                  ...
                  {!showMyQr ? (
                    ...
                    <div className='bg-[#1D1F26] border border-cyan-500/15 rounded-3xl p-6 text-center space-y-6'>
                      ...
                      <div className='w-56 h-56 mx-auto border border-zinc-800 rounded-3xl relative overflow-hidden bg-[#0B0B0F] flex items-center justify-center shadow-inner'>
                        ...
                        {scanning && (
                          <div
                            className='absolute left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-cyan-400 to-transparent shadow-[0_0_10px_rgba(6,182,212,0.8)] animate-scan-line top-0'
                            style={{
                    ...
                    <div className='bg-[#1D1F26] border border-[#D4AF37]/20 rounded-3xl p-6 text-center space-y-6'>
                      ...
                      <p className='text-[11px] text-zinc-400 font-light max-w-xs mx-auto leading-relaxed'>
                        This secure ledger passport uniquely logs your customer
                        profile inside I AM CHOSEN unilevel blockchain systems.
                      </p>
                    </div>
                  )}
                </motion.div>
                      ...
                      {notifications.map((notif) => (
                        ...
                        >
                          <div className='flex justify-between items-start gap-4 mb-1.5'>
                            ...
                            <span className='text-[9px] text-zinc-500 font-mono tracking-wider shrink-0 uppercase'>
                              {notif.date}
                            </span>
                          </div>
                ...
                >
                  ...
                  <div className='bg-[#1D1F26] border border-zinc-800 rounded-3xl p-6 space-y-3.5'>
                    ...
                    {[
                      ...
                      {
                        label: 'Mobile Contact Key',
                        value: userProfile.mobileNumber || 'N/A',
                      },
                      {
                        label: 'Identity Class Role',
                        value: userProfile.role,
                        highlight: true,
                      },
                      {
                        label: 'KYC Level Status',
                        value: userProfile.kycStatus || 'Unverified',
                        badge: true,
                      },
                      {
                        label: 'Account Creation Epoch',
                      ...
                      >
                        ...
                        {row.badge ? (
                          ...
                          <span
                            className={`font-semibold ${row.highlight ? 'text-gold' : 'text-zinc-300'}`}
                          >
                            {row.value}
                          </span>
                        )}
          ...
          <div className='fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm'>
            ...
            >
              ...
              <div className='space-y-4 py-2'>
                <p className='text-zinc-400 text-xs font-light leading-relaxed'>
                  Need help with product deliveries, payments, or your secure
                  credits ledger? Reach out to our compliant support executives:
                </p>
                ...
                <div className='space-y-2.5'>
                  <div className='flex items-center gap-3 bg-[#0B0B0F] p-3.5 rounded-2xl border border-zinc-900'>
                    ...
                    <div>
                      <span className='block text-[9px] uppercase font-bold text-zinc-500 tracking-wide'>
                        Compliant Email Channel
                      </span>
                      <span className='text-xs text-white font-mono select-all'>
                        support@iamchosenintl.com
                      </span>
                    </div>
                  </div>
                ...
                <p className='text-[10px] text-zinc-500 leading-normal italic text-center pt-2'>
                  Compliant inquiries are resolved within standard corporate
                  cycles (1-3 business hours).
                </p>
              </div>
