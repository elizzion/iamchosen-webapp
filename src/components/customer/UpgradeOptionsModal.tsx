...
import { ShoppingBag, Crown, Check, X, Sparkles, Loader2 } from 'lucide-react';
  ...
  return (
    <div
      ...
      className={`relative flex flex-col justify-between rounded-3xl border p-6 transition-all duration-300 group ${
        disabled || isNavigating ? 'opacity-70 cursor-not-allowed' : 'cursor-pointer'
      } ${
        isSelected
          ? isEmerald
...
const UPGRADE_OPTIONS: UpgradeOption[] = [
  ...
  },
  {
    ...
    description:
      'Access the I AM CHOSEN business system, retail-profit opportunities, referral bonuses, leadership rewards, analytics, and business tools.',
    benefits: [
...
export default function UpgradeOptionsModal({
  ...
  const [selectedPath, setSelectedPath] = useState<'smart-customer' | 'affiliate-business' | null>(null);
  ...
  const handleSelectOption = (optionId: 'smart-customer' | 'affiliate-business') => {
