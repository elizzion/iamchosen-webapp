...
function ActiveP2PTransferPage({
  ...
  useEffect(() => {
    if (success) {
      window.showSuccess?.(
        `Successfully transferred ${success.amountCC} CC to ${maskName(success.recipientName)} (ID: ${success.recipientMemberId}). Reference ID: ${success.transferId}.`,
        'Transfer Successful',
      )
    }
  }, [success])
  ...
  const configuredTransferFee = Number(ccSettings.transferFeeAmountCC)
  const isTransferFeeConfigValid =
    ccSettings.transferFeeEnabled !== false &&
    Number.isFinite(configuredTransferFee) &&
    configuredTransferFee === 1
  const TRANSFER_FEE = isTransferFeeConfigValid ? configuredTransferFee : 0
    ...
    const timer = setTimeout(async () => {
      ...
      try {
        ...
        if (userProfile && userProfile.memberId === cleanId) {
          setRecipientError(
            'You cannot transfer credits to your own member ID.',
          )
  ...
  const totalDebit =
    parsedAmount > 0 ? Number((parsedAmount + TRANSFER_FEE).toFixed(4)) : 0
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    if (!isTransferFeeConfigValid) {
      setError(
        'Transfer service is unavailable because the Platform Transfer Fee configuration could not be verified.',
      )
      return
    }
    ...
    if (currentCCBalance < totalDebit) {
      setError(
        `Insufficient balance. You need ${totalDebit} CC (Transfer + Platform Transfer Fee) but currently have ${currentCCBalance} CC.`,
      )
      return
    }
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : 'Failed to complete P2P transfer.',
      )
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div
      className='bg-black min-h-screen text-white flex flex-col selection:bg-gold selection:text-black'
      id='p2p-transfer-page'
    >
      {/* Header Bar */}
      <header className='border-b border-zinc-900 bg-black/80 backdrop-blur sticky top-0 z-40 px-6 py-4 flex items-center justify-between'>
        <button
          onClick={handleBack}
          className='flex items-center gap-2 text-zinc-400 hover:text-white transition-all text-xs tracking-wider uppercase group'
        ...
        <div className='flex items-center gap-2'>
          ...
          <div className='text-right'>
            <span className='text-[9px] text-zinc-500 tracking-widest block leading-none'>
              I AM CHOSEN
            </span>
            <span className='text-[8px] text-gold tracking-[0.3em] block mt-0.5 uppercase leading-none'>
              TRANSFER GATE
            </span>
          </div>
        </div>
        ...
        <AnimatePresence mode='wait'>
          {!success ? (
              ...
              <div className='lg:col-span-7 space-y-6'>
                <div>
                  <h1 className='text-2xl font-black uppercase tracking-tight text-white gold-text flex items-center gap-2'>
                    <ArrowUpRight className='w-6 h-6 text-gold' /> Transfer
                    Chosen Credits
                  </h1>
                  <p className='text-xs text-zinc-400 mt-1 font-light'>
                    Instantly transfer Chosen Credits (CC) to other registered
                    community members. Transfers are submitted to the secured
                    wallet service and are completed only after server-side
                    validation.
                  </p>
                </div>
                ...
                {error && (
                  <div
                    className='bg-red-500/10 border border-red-500/20 text-red-400 p-4 rounded-xl text-xs flex items-start gap-2 animate-fadeIn'
                    id='transfer-error'
                  >
                ...
                {!isTransferFeeConfigValid && (
                  <div
                    className='rounded-xl border border-amber-500/20 bg-amber-500/10 p-4 text-xs text-amber-300'
                    id='transfer-config-warning'
                  >
                    The Platform Transfer Fee configuration could not be
                    verified. Transfers are disabled to protect member funds.
                  </div>
                )}

                <form
                  onSubmit={handleTransferSubmit}
                  className='space-y-6 bg-zinc-950/40 border border-zinc-900/60 rounded-2xl p-6'
                >
                  {/* Step 1: Recipient Search */}
                  <div className='space-y-2'>
                    <label className='block text-[10px] tracking-widest uppercase text-zinc-400 font-bold'>
                    ...
                    <div className='relative'>
                      <input
                        ...
                        disabled={loading || !isTransferFeeConfigValid}
                        className='w-full bg-zinc-950 border border-zinc-800/80 rounded-xl px-4 py-3 pl-10 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-gold transition-all'
                        required
                    ...
                    {verifiedRecipient && (
                      <div
                        className='bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-3 flex items-center justify-between animate-fadeIn'
                        id='recipient-verified-panel'
                      >
                        <div className='flex items-center gap-2'>
                        ...
                        <span className='px-2 py-0.5 bg-emerald-500/20 text-emerald-300 text-[9px] rounded font-bold uppercase tracking-wider'>
                          Active User
                        </span>
                      </div>
                    )}
                    ...
                    {recipientError && (
                      <div
                        className='bg-red-500/5 border border-red-500/15 rounded-xl p-3 text-xs text-red-400 animate-fadeIn'
                        id='recipient-verify-error'
                      >
                        {recipientError}
                      </div>
                    )}
                  ...
                  <div className='space-y-2'>
                    <div className='flex justify-between items-center'>
                      ...
                      <span className='text-[9px] text-zinc-500'>
                        Min: {MIN_TRANSFER} CC | Max:{' '}
                        {MAX_TRANSFER.toLocaleString()} CC
                      </span>
                    </div>
                    <div className='relative'>
                      <input
                        ...
                        disabled={loading || !isTransferFeeConfigValid}
                        className='w-full bg-zinc-950 border border-zinc-800/80 rounded-xl px-4 py-3 pl-12 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-gold transition-all'
                        required
                        id='transfer-amount-input'
                      />
                      <span className='text-zinc-500 text-sm absolute left-4 top-3.5'>
                        CC
                      </span>
                    </div>
                  </div>
                  ...
                  <div className='space-y-2'>
                    ...
                    <textarea
                      ...
                      maxLength={100}
                      rows={2}
                      className='w-full bg-zinc-950 border border-zinc-800/80 rounded-xl p-4 text-xs text-white placeholder-zinc-600 focus:outline-none focus:border-gold transition-all resize-none'
              ...
              <div className='lg:col-span-5 space-y-6'>
                ...
                <div className='bg-zinc-950 border border-zinc-900 rounded-2xl p-6 space-y-6'>
                  <div className='flex items-center gap-3 pb-4 border-b border-zinc-900'>
                    ...
                    <div>
                      <span className='text-[9px] text-zinc-500 tracking-wider block'>
                        CHOSEN WALLET BALANCE
                      </span>
                      {loadingWallet ? (
                        <div className='w-20 h-4 bg-zinc-800 animate-pulse rounded mt-1' />
                      ) : (
                        <span className='text-xl font-black text-white leading-none mt-1 block'>
                          {currentCCBalance.toLocaleString(undefined, {
                  ...
                  <div className='space-y-4'>
                    <h3 className='text-[10px] tracking-widest uppercase text-zinc-400 font-bold'>
                    ...
                    <div className='space-y-2.5 text-xs text-zinc-400'>
                      <div className='flex justify-between'>
                        ...
                        <span className='text-white font-bold'>
                          {parsedAmount.toFixed(2)} CC
                        </span>
                      </div>
                      <div className='flex justify-between items-center'>
                        <span className='flex items-center gap-1'>
                          Platform Transfer Fee:
                          <span className='px-1.5 py-0.5 bg-gold/10 text-gold text-[8px] rounded border border-gold/20 font-bold'>
                            {TRANSFER_FEE.toFixed(2)} CC
                          </span>
                        </span>
                        <span className='text-white font-bold'>
                          {TRANSFER_FEE.toFixed(2)} CC
                  ...
                  <div className='bg-zinc-950/60 border border-zinc-900 rounded-xl p-4 space-y-2'>
                    <div className='flex items-center gap-1.5 text-gold text-[9px] font-bold tracking-widest uppercase'>
                      <Sparkles className='w-3.5 h-3.5' /> Idempotency Trail
                    </div>
                    <p className='text-[10px] text-zinc-500 font-light leading-relaxed'>
                      Every transaction is linked to a pre-generated, unique
                      idempotency key. This ensures that even if you
                      double-click or lose connection, your funds are protected
                      from double debits.
                    </p>
                    <div className='text-[9px] text-zinc-400 bg-zinc-950 px-2 py-1.5 rounded border border-zinc-900 overflow-hidden text-ellipsis whitespace-nowrap'>
                ...
                <div className='bg-zinc-950/30 border border-zinc-900/50 rounded-2xl p-5 text-[11px] text-zinc-500 leading-relaxed font-light space-y-2'>
                  <div className='text-white uppercase font-bold tracking-wider text-[10px]'>
                    Security Notice
                  </div>
                  <p>
                    Once executed, completed P2P credit transfers are
                    irreversible and final. Please double-check the recipient's
                    member ID and verified masked name before submitting.
                  </p>
                  <p>
                    The 1.00 CC transfer fee is deducted directly from your
                    Chosen Wallet balance upon successful settlement.
                  </p>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              {/* Receipt Data Box */}
              <div className='bg-zinc-950 border border-zinc-900 rounded-2xl p-5 text-left text-xs space-y-3 text-zinc-400'>
                <div className='flex justify-between'>
                  <span>Sender Account:</span>
                  <span className='text-white'>
                    {formatMemberId(success.senderMemberId)}
                  </span>
                </div>
                <div className='flex justify-between'>
                  <span>Recipient Account:</span>
                  <span className='text-white'>
                    {formatMemberId(success.recipientMemberId)}
                  </span>
                </div>
                <div className='flex justify-between'>
                  <span>Recipient Name:</span>
                  <span className='text-white text-right font-bold'>
                    {maskName(success.recipientName)}
                  </span>
                </div>
                <div className='border-t border-zinc-900 my-2 pt-2.5 flex justify-between text-sm text-white font-bold'>
                  <span>Amount Transferred:</span>
                  <span className='gold-text font-black'>
                    {success.amountCC.toFixed(2)} CC
                  </span>
                </div>
                <div className='flex justify-between text-[11px] text-zinc-500'>
                ...
                <div className='flex justify-between text-[10px] text-zinc-500 pt-1'>
                  ...
                  <span>{formatCompletedAt(success.createdAt)}</span>
                </div>
              </div>
