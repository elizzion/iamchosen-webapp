...
export function AffiliateGrowthToolsSection({ userProfile }: AffiliateGrowthToolsSectionProps) {
...
export function AffiliateReferralLinkCard({ sponsorCode }: AffiliateReferralLinkCardProps) {
  ...
  const handleCopy = async () => {
    ...
    try {
      ...
      setCopyError('We could not copy the referral link. Please select and copy it manually.');
    }
  };
        ...
        <div className="flex-1 max-w-full lg:max-w-xl">
          {cleanSponsorCode ? (
            ...
            <div className="flex items-center gap-2 text-amber-500 text-xs bg-amber-500/5 border border-amber-500/10 rounded-xl p-3">
              ...
              <span>Your referral link is temporarily unavailable. Missing sponsor code.</span>
            </div>
          )}
...
function CommunityButton({ platform, label, url, icon: Icon }: CommunityButtonProps) {
