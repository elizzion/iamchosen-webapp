...
// Use the configured Firestore database ID when present. The Cloud Functions
// deployment must use the same FIRESTORE_DATABASE_ID value.
const configuredFirestoreDatabaseId = String(
  firebaseConfig.firestoreDatabaseId || '',
).trim()
export const db: Firestore = configuredFirestoreDatabaseId
  ? getFirestore(app, configuredFirestoreDatabaseId)
  : getFirestore(app)
...
export type PackageActivationAction =
  | 'INITIAL_ACTIVATION'
  | 'PACKAGE_UPGRADE'
  | 'BUSINESS_CYCLE_REACTIVATION'

export interface ActivatePackageWithWalletRequest {
  packageId: string
...
export interface ActivatePackageWithWalletResponse {
  success: true
...
export const activatePackageWithWallet = httpsCallable<
  ActivatePackageWithWalletRequest,
  ActivatePackageWithWalletResponse
>(functions, 'activatePackageWithWallet')

export const processMsaCreditsNow = httpsCallable<
  Record<string, never>,
  {
    success: boolean
    processed: number
...
>(functions, 'processMsaCreditsNow')

const recordClientAuditEvent = httpsCallable<
  { action: string; details: string },
  { success: boolean; logId: string }
>(functions, 'recordClientAuditEvent')
...
async function testConnection() {
  try {
    ...
    if (error instanceof Error && error.message.includes('client is offline')) {
      console.error(
        'Please check your Firebase configuration or connection. The client is offline.',
      )
    }
  }
}
...
// Create a non-authoritative client event through the secured callable.
// Financial and system audit logs are created only by trusted backend functions.
export async function createAuditLog(
  ...
  try {
    await recordClientAuditEvent({ action, details })
  } catch (error) {
    // Client telemetry must never make the originating user action appear to
    // fail after the authoritative backend transaction has already completed.
    console.error('Failed to record client audit event', error)
  }
}
...
export async function verifySponsorCode(
  ...
  try {
    const q = query(
      collection(db, 'users'),
      where('sponsorCode', '==', code.toUpperCase().trim()),
      limit(1),
    )
...
export async function ensureUserProfile(
  ...
  const fullName =
    firebaseUser.displayName || email.split('@')[0] || 'Valued Member'
  const sponsorCode = generateSponsorCode(fullName)
  const timestamp = new Date().toISOString()

  const isSuperAdmin =
    email.trim().toLowerCase() === 'nifled.kenjaktrading@gmail.com'
...
export async function registerNewMemberBySponsor(
  ...
  await runTransaction(db, async (transaction) => {
    ...
    if (!sponsorWalletSnap.exists()) {
      throw new Error(
        'Sponsor wallet not found. Please activate your account first.',
      )
    }
      ...
      if (sponsorCycleData.status === 'Active') {
        ...
        const updatedCycle: Partial<BusinessCycle> = {
          currentQualifiedEarningsCC: Number(
            (currentEarnings + creditedCommission).toFixed(2),
          ),
          remainingCapacityCC: Number(
            (remainingCapacity - creditedCommission).toFixed(2),
          ),
    ...
    const updatedSponsorWallet: Partial<Wallet> = {
      chosenWalletBalance: Number(
        (currentSponsorBalance - packageCost).toFixed(2),
      ),
      commissionWalletBalance: Number(
        (
          (sponsorWalletData.commissionWalletBalance || 0) + creditedCommission
        ).toFixed(2),
      ),
      updatedAt: new Date().toISOString(),
    }
    ...
    if (sponsorCycleCompleted) {
      transaction.update(sponsorProfileRef, {
        status: 'Completed',
        commissionEligible: false,
        updatedAt: new Date().toISOString(),
      })
    ...
    transaction.set(
      doc(db, 'wallet_transactions', txSponsorDebitId),
      txSponsorDebit,
    )
    ...
    if (creditedCommission > 0) {
      ...
      transaction.set(
        doc(db, 'wallet_transactions', txSponsorCreditId),
        txSponsorCredit,
      )
    }
    ...
    if (isFlushed && adminUid) {
      ...
      transaction.update(doc(db, 'wallets', adminUid), {
        commissionWalletBalance: Number(
          (currentAdminBalance + commissionAmount).toFixed(2),
        ),
        updatedAt: new Date().toISOString(),
      })
      ...
      transaction.set(
        doc(db, 'flushed_commissions', flushedCommissionId),
        flushedRecord,
      )
      ...
      const memberNotif = {
        ...
        message:
          'Your commission eligibility is paused because your Business Cycle is completed. Reactivate or upgrade to continue earning.',
        type: 'warning',
        unread: true,
        createdAt: timestamp,
      }
...
export async function approvePendingAffiliate(
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const pkg =
      packages[packageLevel as keyof typeof packages] || packages.Bronze
    const timestamp = new Date().toISOString()
...
export const submitPackageCashInRequest = httpsCallable<any, any>(
  functions,
  'submitPackageCashInRequest',
)
export const approveCashInAndActivatePackage = httpsCallable<any, any>(
  functions,
  'approveCashInAndActivatePackage',
)
export const previewPackagePurchase = httpsCallable<any, any>(
  functions,
  'previewPackagePurchase',
)

/** @deprecated Use activatePackageWithWallet. */
export const purchasePackageWithWallet = httpsCallable<any, any>(
  functions,
